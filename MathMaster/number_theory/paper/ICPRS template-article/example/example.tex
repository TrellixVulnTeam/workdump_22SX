...
The RSA algorithm is one of the earliest implementations of an asymmetric cryptosystem \cite{diffie1976new}. Asymmetric encryption differs from the classical encryption as it uses separate keys for encryption (public key) and decryption (private key) \cite{???}. This allows the users to establish a private communication over a public channel. Furthermore, it secures sensitive data from so-called “man-in-the-middle“ attacks wherein malicious third-party eavesdrops on the communication between two parties with the intent of stealing sensitive information \cite{???}. Another key feature of RSA is the ability to create digital signatures. Similar to a physical signature, an digital signature provides a way to prove that a message originated from the sender along with the means to show that the message was not tampered with \cite{diffie1976new}. 

// We will discuss how RSA: (a) scramble or encrypt a message into ciphertext, (b) unscramble or decrypt ciphertext back to the plaintext, (c) generate digital signatures. THEN We will discuss how to establish the correctness of RSA ... relies on Euler’s theorem along with the properties of congruences to perform its various cryptographic procedures. THEN sample scenario to illustrate stuff ...
