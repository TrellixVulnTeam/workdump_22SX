...
$Distance[d(x, y), x, y, G] \abr d(x, y) = min({n \in \mathbb{N} \st (\exists_{V_n}(Path[V_n, n, G]) \land ((v_1, v_{n + 1}) = (x, y)))})$ \\
...
\end{array}\right)}$ \\
\begin{enumerate}
  \lit By definition of cardinality and sets, $d(x, y) \geq 0$, $(d(x, y) = 0) \iff (x = y)$, $d(x, y) = d(y, x)$
  \lit By cases:
  \begin{enumerate}
