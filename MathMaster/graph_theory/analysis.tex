...
$SimpleGraph[(V, E)] \abr (Graph[(V, E)]) \land (E \subseteq \{\utup{a, b} \in V^{\{2\}} \st a \neq b\})$ \\
...
$CompleteGraph[K_n, n] \abr (|K_n| = n) \land (e(K_n) = {n \choose 2})$ \\
$EmptyGraph[E_n, n] \abr (|K_n| = n) \land (e(K_n) = 0)$ \\
$TrivialGraph[G] \abr G = K_1 = E_1$ \\
$ComplementGraph[\bar{G}, G] \abr \bar{G} = (V, V^{\{2\}} \setminus (E \cup \{\utup{x, x} \st x \in V(G)\}))$ \\
...
$MinDegree[\delta(G), G] \abr \delta(G) = min(\{d_G(x) \st x \in V(G)\})$ \\
$MaxDegree[\Delta(G), G] \abr \Delta(G) = max(\{d_G(x) \st x \in V(G)\})$ \\
...
$DegreeSequence[(d_G(x_i))_1^n, G] \abr (Order[n, G]) \land (((d_G(x_i))_1^n) = sort(\{d_G(x) \st x \in V(G)\})) \land (\delta(G) = d_G(x_1) \leq d_G(x_n) = \Delta(G))$ \\

$SumDegrees \abr \sum_{v \in V(G)}(d_G(v)) = 2 e(G)$ \\
\begin{enumerate}
  \lit $\sum_{v \in V(G)}(d_G(v)) = \sum_{v \in V(G)}(|\{e \in E(G) | v \in e\}|) = 2 |E(G)| = 2 e(G)$ \\
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\

$HandshakingLemma \abr \sum_{v \in V(G)}(d_G(v)) \equiv 0 \pmod{2}$ \\
\begin{enumerate}
  \lit SumDegrees \pipe $\sum_{v \in V(G)}(d_G(v)) = 2 e(G)$ \pipe $\exists_{k \in \mathbb{Z}}(\sum_{v \in V(G)}(d_G(v)) - 0 = 2 k)$ \pipe $\sum_{v \in V(G)}(d_G(v)) \equiv 0 \pmod{2}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\

$DegreeCorollaries \abr (Even(|\{v \in V(G) \st Odd(d_G(v))\}|)) \land (\delta(G) \leq \floor{2 e(G) / n}) \land (\Delta(G) \geq \ceil{2 e(G) / n})$
\begin{enumerate}
  \lit $HandshakingLemma$ \pipe $Even(|\{v \in V(G) \st Odd(d_G(v))\}|)$
...
$IndependentPath[\mathcal{P}, G] \abr \exists_{x, y \in V(G)} \forall_{P, Q \in \mathcal{P}}((P \neq Q) \implies (V(P) \cap V(Q) = \utup{x, y}))$ \\
$IndependentVEquiv \abr IndependentV \iff SubgraphInducedByV[] \cong E_n$

$PathG[P_n, n] \abr (V(P_n) = \{x_i \st i \in \mathbb{N}_1^{n}\}) \land (E(P_n) = \{\utup{x_i, x_{i + 1}} \st i \in \mathbb{N}_1^{n - 1}\})$ \\
$CycleG[C_n, n] \abr (V(C_n) = V(P_ n)) \land (E(C_n) = E(P_n) \cup \{\utup{x_n, x_1}\})$ \\

$CyclePartition \abr (\forall_{v \in V(G)}(Even(d_G(v)))) \iff (\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)])))$ \\
\begin{enumerate}
  \lit $(\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)]))) \implies \ldots$
  \begin{enumerate}
    \lit $\forall_{v \in V(G)}(d_G(v) = 2 * |\{v \st (C \in \mathcal{C}) \land (v \in C)\})|$ \pipe $\forall_{v \in V(G)}(Even(d_G(v)))$
  \end{enumerate}
  \lit $(\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)]))) \implies (\forall_{v \in V(G)}(Even(d_G(v))))$
  \lit $(\forall_{v \in V(G)}(Even(d_G(v)))) \implies \ldots$
  \begin{enumerate}
    \lit $(e(G) = 0) \implies (\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)])))$
    \lit $(e(G) \neq 0) \implies \ldots$
    \begin{enumerate}
      \lit $(e(G) > 0) \land (\forall_{v \in V(G)}(Even(d_G(v))))$ \pipe $\exists_{x_0 \in V(G)}(d_G(x_0) \geq 2)$
      \lit There exists a PathG $P$ of maximal length with endvertices $(x_0, x_l)$.
      \lit $(d_G(x_0) \geq 2)$ \pipe Let $y$ be another vertex adjacent to $x_0$ that is not $x_1$.
    ...
    \lit $\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)]))$
  \end{enumerate}
  \lit $(\forall_{v \in V(G)}(Even(d_G(v)))) \implies (\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)])))$
  \lit $(\forall_{v \in V(G)}(Even(d_G(v)))) \iff (\exists_{\mathcal{C}}((\mathcal{E} = \{C_E \st (C \in \mathcal{C}) \land (CycleE[C_E, (C, G)])\}) \land (Partition[\mathcal{E}, E(G)])))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$Girth[G] \abr min(\{n \in \mathbb{N} \st \exists_{V_n}(Cycle[V_n, n, G])\})$ \\
$Circumference[G] \abr max(\{n \in \mathbb{N} \st \exists_{V_n}(Cycle[V_n, n, G])\})$ \\

$ConnectedV[(x, y), G] \abr \exists_{n, V_n}((Path[V_n, n, G]) \land ((x, y) = (v_1, v_{n + 1})))$ \\
$Connected[G] \abr \forall_{x, y \in V(G)}(ConnectedV[(x, y), G])$ \\
$Component[C, G] \abr (Subgraph[C, G]) \land (\forall_{c \in V(C)} \forall_{g \in V(G)}((g \notin V(C)) \implies (\lnot ConnectedV[(g, c), G])))$ \\

$Degree[d_G(v), v, G] \abr d_G(v) = |\{e \in E(G) | v \in e\}|$ \\
$Regular[G, r] \abr \forall_{v \in V(G)}(d_G(v) = r)$ \\

$SumDeg \abr \sum_{v \in V(G)}(d_G(v)) = 2 |E(G)|$ \\
\begin{enumerate}
  \lit $\sum_{v \in V(G)}(d_G(v)) = \sum_{v \in V(G)}(|\{e \in E(G) | v \in e\}|) = 2 |E(G)|$ \\
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\

$OddDeg \abr Even(|\{v \st Odd(d_G(v))\}|)$ \\
\begin{enumerate}
  \lit SumDeg
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\
...
$CompleteG[K_n, n] \abr (V = V(P_n)) \land (E = \{\utup{v_i, v_j} \st (i, j \in \mathbb{N}_1^n) \land (i \neq j)\})$ \\
...
$Distance[d_G(x, y), x, y, G] \abr d_G(x, y) = min({n \in \mathbb{N} \st (\exists_{V_n}(Path[V_n, n, G]) \land ((v_1, v_{n + 1}) = (x, y)))})$ \\
$DMetric \abr \forall_{G, x, y, z}\pr{((Graph[G]) \land (x, y, z \in V(G))) \implies 
\left(\begin{array}{ll}
  (d_G(x, y) \geq 0) \hfill \land \hfill \\
  ((d_G(x, y) = 0) \iff (x = y)) \hfill \land \hfill \\
  (d_G(x, y) = d(y, x)) \hfill \land \hfill \\
  (d_G(x, y) + d_G(y, z) \geq d_G(x, z)) \\
\end{array}\right)}$ \\
\begin{enumerate}
  \lit By definition of cardinality and sets, $d_G(x, y) \geq 0$, $(d_G(x, y) = 0) \iff (x = y)$, $d_G(x, y) = d(y, x)$
  \lit By cases:
  \begin{enumerate}
...
$GraphPower[G^r, r, G] \abr (V = V(G)) \land (E = \{\utup{x, y} \st d_G(x, y) \leq r\}) \land (G^r = (V, E))$ \\
...
$Tree[G] \abr (Connected[G]) \land (\lnot \exists_{n, V_n}(Cycle[V_n, n, G]))$ \\
