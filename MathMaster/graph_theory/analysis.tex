...
$EvenCycle[W, G] \abr (CycleW[W, G]) \land (Even(|W| - 1))$ \\
$OddCycle[W, G] \abr (CycleW[W, G]) \land (Odd(|W| - 1))$ \\
$Triangle[W, G] \abr (CycleW[W, G]) \land (|W| - 1 = 3)$ \\
...
$Path[P, V] \abr (V(P) = V) \land (E(P) = \{\utup{v_i, v_{i + 1}} \st i \in \mathbb{N}_1^{|V| - 1}\})$ \\
$CycleG[C_n, n] \abr (V(C_n) = V(P_ n)) \land (E(C_n) = E(P_n) \cup \{\utup{x_n, x_1}\})$ \\
$PathInG[P, V, G] \abr (Path[P, V]) \land (V \subseteq V(G))$ \\
...
$Component[C, G] \abr (Subgraph[C, G]) \land (\lnot \exists_{D}((Subgraph[D, G]) \land (Subgraph[C, D])))$ \\
page 21
