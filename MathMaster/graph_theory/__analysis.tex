...
$EvenCycle[W, G] \abr (CycleW[W, G]) \land \pr{Even(|W| - 1)}$ \\
$OddCycle[W, G] \abr (CycleW[W, G]) \land \pr{Odd(|W| - 1)}$ \\
...
$Path[P, V] \abr \pr{V(P) = V} \land \pr{E(P) = \{\utup{v_i, v_{i + 1}} \st i \in \mathbb{N}_1^{|V| - 1}\}}$ \\
$CycleG[C_n, n] \abr \pr{V(C_n) = V(P_ n)} \land \pr{E(C_n) = E(P_n) \cup \{\utup{x_n, x_1}\}}$ \\
$PathInG[P, V, G] \abr (Path[P, V]) \land \pr{V \subseteq V(G)}$ \\
...
$Component[C, G] \abr (Subgraph[C, G]) \land \pr{\lnot \exists_{D}\pr{(Subgraph[D, G]) \land (Subgraph[C, D])}}$ \\
page 21
