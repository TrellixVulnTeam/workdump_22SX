...
$PathInWalk[P, W, G] \abr (VPath[P, G]) \land (VWalk[W, G]) \land \exists_{\phi}\pr{(OrderedDeletion[\phi, W]) \land \pr{P = \phi(W)}}$ \\
$CycleInWalk[C, W, G] \abr (VCycle[C, G]) \land (VWalk[W, G]) \land \exists_{\phi}\pr{(OrderedDeletion[\phi, W]) \land \pr{C = \phi(W)}}$ \\
...
$xyWalkInxyPath \abr \pr{xyWalk[(x, y), W, G]} \implies \pr{\exists_{P}\pr{\pr{xyPath[(x, y), P, G]} \land (PathInWalk[P, W, G])}}$
\begin{enumerate}
  \lit $\pr{l(W) = 0} \implies (P = W)$ \pipe $PathInWalk[P, W, G]$
  \lit $\pr{(l(W) > 0} \land \pr{\forall_{W'}((l(W') < l(W)} \implies \\
        \pr{\pr{xyWalk[(x, y), W', G]} \implies \pr{\exists_{P'}\pr{\pr{xyPath[(x, y), P', G]} \land (PathInWalk[P', W', G])}}}))) \implies \ldots$
  \begin{enumerate}
  ...
  \end{enumerate}
  \lit $\pr{(l(W) > 0} \land \pr{\forall_{W'}((l(W') < l(W)} \implies \\
  \pr{\pr{xyWalk[(x, y), W', G]} \implies \pr{\exists_{P'}\pr{\pr{xyPath[(x, y), P', G]} \land (PathInWalk[P', W', G])}}}))) \implies (PathInWalk[P, W, G])$
  \lit By induction: $\pr{xyWalk[(x, y), W, G]} \implies \pr{\exists_{P}\pr{\pr{xyPath[(x, y), P, G]} \land (PathInWalk[P, W, G])}}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
