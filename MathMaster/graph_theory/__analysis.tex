...
$kColorable[G, k] \abr \exists_{\phi}\pr{(ProperColoring[\phi, G]) \land (kColoring[\phi, k, G])}$ \\
$ChromaticNumber[\chi(G), G] \abr min(\{k \in \mathbb{N} \st kColorable[G, k]\})$ \\

$ColorAssignment[L, G] \abr Function[L, V(G), \mathcal{P}(\mathbb{N})]$ \\
$LColoring[\phi, L, G] \abr (Coloring[\phi, G]) \land (ColorAssignment[L, G]) \land \pr{\forall_{v \in V(G)}(\phi(v) \in L(v)}$ \\
$LColorable[G, L] \abr \exists_{\phi}\pr{(ProperColoring[\phi, G]) \land (LColoring[\phi, L, G])}$ \\
$kChoosable[G, k] \abr \forall_{L}\pr{\pr{(ColorAssignment[L, G]) \land \pr{\forall_{v \in V(G)}\pr{|L(v)| = k}}} \implies (LColorable[G, L])}$ \\
...
$OhbaEquiv \abr \pr{(CompletekPartite[G, k]) \land \pr{n(G) \leq 2 k + 1}} \implies \pr{\chi_l(G) = \chi(G) = k}$ \\
