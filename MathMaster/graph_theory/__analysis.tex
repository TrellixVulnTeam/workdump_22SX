...
$EdgeSet[E\pr{(V, E)}, (V, E)] \abr \pr{GrSimpleGraphaph[(V, E)]} \land \pr{E\pr{(V, E)} = E}$ \\
...
$CompleteBipartiteG[G] \abr (PartiteSets[\{X, Y\}, G]) \land \pr{E(G) = \{\utup{x, y} \st (x \in X) \land (y \in Y)\}}$ \\
...
$VWalk[W, G] \abr \pr{\forall_{i \in \mathbb{N}_1^{|W| - 1}}\pr{\utup{w_i, w_{i + 1}} \in E(G)}}$ \\
$VTrail[W, G] \abr (Walk[W, G]) \land \pr{\forall_{i, j \in \mathbb{N}_1^{|W| - 1}}\pr{(i \neq j) \implies (\utup{w_i, w_{i + 1}} \neq \utup{w_j, w_{j + 1}})}}$ \\
$VPath[W, G] \abr (Walk[W, G]) \land \pr{\forall_{i, j \in \mathbb{N}_1^{|W|}}\pr{(i \neq j) \implies (w_i \neq w_j)}}$ \\
$VCycle[W, G] \abr (Walk[W, G]) \land (w_{|W|} = w_1) \land \pr{\forall_{i, j \in \mathbb{N}_2^{|W| - 1}}\pr{(i \neq j) \implies (w_i \neq w_j)}}$ \\
...
$VLength[len(W), W, G] \abr (Walk[W, G]) \land \pr{l(W) = |W| - 1}$ \\
$VClosed[W, G] \abr (Walk[W, G]) \land (w_0 = w_{|W|})$ \\
$VOdd[W, G] \abr (Walk[W, G]) \land \pr{Odd\pr{len(W)}}$ \\
$VEven[W, G] \abr (Walk[W, G]) \land \pr{Even\pr{len(W)}}$ \\
$PathContainsWalk[P, W, G] \abr (VPath[P, G]) \land (VWalk[W, G]) \land \exists_{\phi}\pr{(OrderedDeletion[\phi, W]) \land \pr{P = \phi(W)}}$ \\
...
$xyWalkInxyPath \abr \pr{xyWalk[(x, y), W, G]} \implies \pr{\exists_{P}\pr{\pr{xyPath[(x, y), P, G]} \land (PathContainsWalk[P, W, G])}}$
\begin{enumerate}
  \lit $\pr{l(W) = 0} \implies (P = W)$ \pipe $PathContainsWalk[P, W, G]$
  \lit $\pr{(l(W) > 0} \land \pr{\forall_{W'}((l(W') < l(W)} \implies \\
        \pr{\pr{xyWalk[(x, y), W', G]} \implies \pr{\exists_{P'}\pr{\pr{xyPath[(x, y), P', G]} \land (PathContainsWalk[P', W', G])}}}))) \implies \ldots$
  \begin{enumerate}
  ...
  \end{enumerate}
  \lit $\pr{(l(W) > 0} \land \pr{\forall_{W'}((l(W') < l(W)} \implies \\
  \pr{\pr{xyWalk[(x, y), W', G]} \implies \pr{\exists_{P'}\pr{\pr{xyPath[(x, y), P', G]} \land (PathContainsWalk[P', W', G])}}}))) \implies (PathContainsWalk[P, W, G])$
  \lit By induction: $\pr{xyWalk[(x, y), W, G]} \implies \pr{\exists_{P}\pr{\pr{xyPath[(x, y), P, G]} \land (PathContainsWalk[P, W, G])}}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$ConnectedV[(x, y), G] \abr \exists_{P}\pr{xyPath[(x, y), P, G]}$ \\
...
\end{array}\right)$
\begin{enumerate}
  \lit By $(xyWalkInxyPath) \land \pr{xyPath[(x, y), W, G]} \iff \pr{xyPath[(y, x), W, G]}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
\begin{enumerate}
  ...
  \lit WTS: $(Connected[H - e]) \iff \pr{\exists_{C}\pr{\pr{(CycleG[C]) \land Subgraph[C, G]} \land \pr{e \in E(C)}}}$
  \lit $(Connected[H - e]) \implies \ldots$
  \begin{enumerate}
    \lit $\exists_{P}\pr{(PathG[P]) \land (Subgraph[P, H - e])}$ \pipe $P + e$ must contain a cycle \pipe $\exists_{C}\pr{\pr{(CycleG[C]) \land Subgraph[C, G]} \land \pr{e \in E(C)}}$
  \end{enumerate}
  \lit $(Connected[H - e]) \implies \pr{\exists_{C}\pr{\pr{(CycleG[C]) \land Subgraph[C, G]} \land \pr{e \in E(C)}}}$
  \lit $\pr{\exists_{C}\pr{\pr{(CycleG[C]) \land Subgraph[C, G]} \land \pr{e \in E(C)}}} \implies \ldots$
  \begin{enumerate}
    \lit $Component[H, G]$ \pipe $Connected[H]$
    \lit $\pr{u, v \in V(H)} \implies \ldots$
    \begin{enumerate}
      \lit $\exists_{P}\pr{xyPath[(u, v), P, H]}$
      \lit $\pr{e \notin E(P)} \implies \pr{\exists_{P}\pr{xyPath[(u, v), P, H]}}$
      \lit $\pr{e \in E(P)} \implies \ldots$
      \begin{enumerate}
        \lit $P' = u-x Path + x-y CycleG + y-v Path$
        \lit $xyPath[(u, v), P', H - e]$ \pipe $\exists_{P}\pr{xyPath[(u, v), P, H]}$
      \end{enumerate}
      \lit $\pr{e \in E(P)} \implies \pr{\exists_{P}\pr{xyPath[(u, v), P, H]}}$
      \lit $\exists_{P}\pr{xyPath[(u, v), P, H]}$
    \end{enumerate}
    \lit $\pr{u, v \in V(H)} \implies \pr{\exists_{P}\pr{xyPath[(u, v), P, H - e]}}$ \pipe $Connected[H - e]$
  \end{enumerate}
  \lit $\pr{\exists_{C}\pr{\pr{(CycleG[C]) \land Subgraph[C, G]} \land \pr{e \in E(C)}}} \implies (Connected[H - e])$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$OddCycleInClosedOddWalk \abr \pr{(VClosed[W, G]) \land (VOdd[W, G])} \implies \pr{\exists_{C}\pr{(VCycle[C, G]) \land (VOdd[C, G]) \land (CycleInWalk[C, W, G])}}$
\begin{enumerate}
  \lit $\pr{len(W) = 1} \implies (W = C)$ \pipe $\exists_{C}\pr{(VCycle[C, G]) \land (VOdd[C, G]) \land (CycleInWalk[C, W, G])}$
  \lit $\pr{(len(W) > 1} \land \pr{\forall_{W'}((len(W') < len(W)} \implies \\
        \pr{\pr{(VClosed[W', G]) \land (VOdd[W', G])} \implies \pr{\exists_{C'}\pr{(VCycle[C', G]) \land (VOdd[C', G]) \land (CycleInWalk[C', W', G])}}}))) \implies \ldots$
  \begin{enumerate}
    \lit If $W$ has no repeated vertex other than the first and last, then $C = W$ \pipe $\exists_{C}\pr{(VCycle[C, G]) \land (VOdd[C, G]) \land (CycleInWalk[C, W, G])}$
    ...
    \lit WLOG let $W_1$ be the smaller odd walk, then by IH $\exists_{C'}\pr{(VCycle[C', G]) \land (VOdd[C', G]) \land (CycleInWalk[C', W_1, G])}$
    \lit $\exists_{C}((VCycle[C, G]) \land (VOdd[C, G]) \land (CycleInWalk[C, W, G])$
  \end{enumerate}
  \lit $\pr{(len(W) > 1} \land \pr{\forall_{W'}((len(W') < len(W)} \implies \\
        \pr{\pr{(VClosed[W', G]) \land (VOdd[W', G])} \implies \pr{\exists_{C'}\pr{(VCycle[C', G]) \land (VOdd[C', G]) \land (CycleInWalk[C', W', G])}}}))) \implies \\
        (\exists_{C}((VCycle[C, G]) \land (VOdd[C, G]) \land (CycleInWalk[C, W, G])$
  \lit By induction: $\exists_{C}\pr{(VCycle[C, G]) \land (VOdd[C, G]) \land (CycleInWalk[C, W, G])}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\
...
$BipartiteEquiv \abr (Bipartite[G]) \iff \pr{\lnot \exists_{C}\pr{(CycleG[C]) \land (Subgraph[C, G]) \land (VOdd[C, G])}}$
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \lit $\lnot \exists_{C}\pr{(CycleG[C]) \land (Subgraph[C, G]) \land (VOdd[C, G])}$
  \end{enumerate}
  \lit $(Bipartite[G]) \implies \pr{\lnot \exists_{C}\pr{(CycleG[C]) \land (Subgraph[C, G]) \land (VOdd[C, G])}}$
  \lit $\pr{\lnot \exists_{C}\pr{(CycleG[C]) \land (Subgraph[C, G]) \land (VOdd[C, G])}} \implies \ldots$
  \begin{enumerate}
  ...
  \end{enumerate}
  \lit $\pr{\lnot \exists_{C}\pr{(CycleG[C]) \land (Subgraph[C, G]) \land (VOdd[C, G])}} \implies (Bipartite[G])$
  \lit $(Bipartite[G]) \iff \pr{\lnot \exists_{C}\pr{(CycleG[C]) \land (Subgraph[C, G]) \land (VOdd[C, G])}}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$CompleteAsBipartiteUnion \abr \pr{\exists_{\mathcal{B}}\pr{(|\mathcal{B}| = k) \land \pr{\forall_{B \in \mathcal{B}}(BipartiteG[B])} \land (UnionG[K_n, \mathcal{B}])}} \iff (n \leq 2^k)$
\begin{enumerate}
  \lit $(k = 1) \implies \ldots$
  \begin{enumerate}
    \lit $\pr{\exists_{\mathcal{B}}\pr{(|\mathcal{B}| = k) \land \pr{\forall_{B \in \mathcal{B}}(BipartiteG[B])} \land (UnionG[K_n, \mathcal{B}])}} \iff (Bipartite[K_n])$
    ...
    \lit $(Bipartite[K_n]) \iff (n \leq 2)$ \pipe $\pr{\exists_{\mathcal{B}}\pr{(|\mathcal{B}| = k) \land \pr{\forall_{B \in \mathcal{B}}(BipartiteG[B])} \land (UnionG[K_n, \mathcal{B}])}} \iff (n \leq 2)$
  \end{enumerate}
  \lit $(k = 1) \implies \pr{\pr{\exists_{\mathcal{B}}\pr{(|\mathcal{B}| = k) \land \pr{\forall_{B \in \mathcal{B}}(BipartiteG[B])} \land (UnionG[K_n, \mathcal{B}])}} \iff (n \leq 2)}$
  \lit $\pr{(k > 1) \land \pr{\forall_{k'}\pr{(k' < k) \implies \pr{\pr{\exists_{\mathcal{B}}\pr{(|\mathcal{B}| = k') \land \pr{\forall_{B \in \mathcal{B}}(BipartiteG[B])} \land (UnionG[K_n, \mathcal{B}])}} \iff (n \leq 2^{k'})}}}} \implies \ldots$
  \begin{enumerate}
    \lit THE SUBGRAPH INDUCED BY X AND Y ARE NECESSARILY COMPLETE!!!!!!!!!!!!
  \end{enumerate}
  \lit $\pr{(k > 1) \land \pr{\forall_{k'}\pr{(k' < k) \implies \pr{\pr{\exists_{\mathcal{B}}\pr{(|\mathcal{B}| = k') \land \pr{\forall_{B \in \mathcal{B}}(BipartiteG[B])} \land (UnionG[K_n, \mathcal{B}])}} \iff (n \leq 2^{k'})}}}} \implies 123123$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
