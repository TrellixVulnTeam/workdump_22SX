...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $x < y$ \pipe $y - x > 0$ \pipe $\exists_{n \in \mathbb{N}^+}(n (y - x) > 1)$ \pipe $n_0 \abr choice(\{n \in \mathbb{N}^+ | n (y - x) > 1\})$
    \lit $1 > 0$ \pipe $\exists_{m \in \mathbb{N}^+}(m (1) > n_0 x)$ \pipe $m_1 \abr choice(\{m \in \mathbb{N}^+ | m (1) > n_0 x\})$ \pipe $m_1 > n_0 x$
    \lit $\exists_{m \in \mathbb{N}^+}(m (1) > -n_0 x)$ \pipe $m_2 \abr choice(\{m \in \mathbb{N}^+ | m (1) > -n_0 x\})$ \pipe $m_2 > -n_0 x$ \pipe $-m_2 < n_0 x$
    \lit $-m_2 < n_0 x < m_1$ \pipe $|m_1 - (-m_2)| \geq 2$
    \lit $\exists_{m \in \mathbb{Z}}(-m_2 < n_0 x < m_1 \land m - 1 \leq n_0 x)$ \pipe $m_0 \abr choice(\{m \in \mathbb{Z} | -m_2 < n_0 x < m_1 \land m - 1 \leq n_0 x\})$ % finite and wide enough???
    \lit 123123
  \end{enumerate}
  \lit 123123
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
