...
$LagrangeTheorem \abr \forall_{G, H}(((Order[n, G, *]) \land (Order[m, H, *]) \land (n, m \in \mathbb{N}) \land (Subgroup[H, G, *])) \implies (Divides[m, n]))$
\begin{enumerate}
...
$ConjugateCentralizersCardinality \abr \forall_{c, g, h \in G}((h = c^{-1} g c) \implies (C(h) = c^{-1} C(g) c))$
\begin{enumerate}
  \lit $(x \in C(h)) \iff (h * x = x * h) \iff ((c^{-1} g c) * x = x * (c^{-1} g c)) \iff (x \in c^{-1} C(g) c)$ \pipe $C(h) = c^{-1} C(g) c$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$ConjugateCentersPartitionsG \abr (g \in G) \implies (o(G) = o(C_g) o(C(g)))$
\begin{enumerate}
  \lit $(ConjugateEqRel) \land (EqRelInducesPartition) \land (ConjugateCentralizersCardinality)$ \pipe $o(G) = o(C_g) o(C(g))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
