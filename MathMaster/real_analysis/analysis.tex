...
%{\mathbin{\ooalign{$\hidewidth\ \bar{#1}\hidewidth$\cr$\phantom{$1}$}}}
...
$Comp[g \circ f, f, g, X, Y, Z] \abr (Func[f, X, Y]) \land (Func[g, Y, Z]) \land (g \circ f = \{\langle x, g(f(x)) \rangle \in X \times Z | x \in X\})$ \\
...
$Image[im(A), A, f, X, Y] \abr (Func[f, X, Y]) \land (A \subseteq X) \land (im(A) = \{f(a) \in Y | a \in A\})$ \\
$Preimage[pim(B), B, f, X, Y] \abr (Func[f, X, Y]) \land (B \subseteq Y) \land (pim(B) = \{a \in X | f(a) \in B\})$ \\
...
$Centralizer[C(g), g, G, *] \abr (Group[G, *]) \land (g \in G) \land (C(g) = \{h \in G | g * h = h * g\})$ \\
...
$DihedralGroup[D_n, *] \abr (D_n = \{a^r * b^s | (r \in \mathbb{N}_{0, n - 1}) \land (s \in \mathbb{N}_{0, 1})\}) \land \left(\begin{array}{ll}
...
$CosetInduceEqRel \abr \forall_{G, H}(((Subgroup[H, G, *]) \land (\sim = \{\langle a, b \rangle | a * b^{-1} \in H\})) \implies ((EqRel[\sim, G]) \land (EqClass[H a, a, \sim, G])))$
\begin{enumerate}
...
$CosetSet[G : H, H, G, *] \abr (Subgroup[H, G, *]) \land (G : H = \{g H | g \in G\})$ \\ % similar to QuotientSet
...
$OrderOrderElProp \abr \forall_{g, G}(((Order[n, G, *]) \land (OrderEl[m, g, G, *])) \implies ((Divides[m, n]) \land (g^n = e)))$
\begin{enumerate}
...
\begin{enumerate}
  ...
  \lit $(\forall_{k \in \mathbb{N}^+}(\phi(g^k) = \phi(g)^k)) \implies \ldots$
  \begin{enumerate}
    \lit $\phi(g^{k + 1}) = \phi(g^k * g) = \phi(g)^k \diamond \phi(g) = \phi(g)^{k + 1}$ \pipe $\phi(g^{k + 1}) = \phi(g)^{k + 1}$
  \end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
  \lit $(\forall_{k \in \mathbb{N}^+}(\phi(g^k) = \phi(g)^k)) \implies (\phi(g^{k + 1}) = \phi(g)^{k + 1})$
  \lit $(\phi(g^1) = \phi(g)^1) \land ((\forall_{k \in \mathbb{N}^+}(\phi(g^k) = \phi(g)^k)) \implies (\phi(g^{k + 1}) = \phi(g)^{k + 1}))$ \pipe $\forall_{n \in \mathbb{N}^+}(\phi(g^n) = \phi(g)^n))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$MapDivProp \abr ((Homomorphism[\phi, G, *, H, \diamond]) \land (Order[n, G, *])) \implies (\forall_{g \in G}((OrderEl[m, \phi(g), H, \diamond]) \implies (Divides[m, n])))$
\begin{enumerate}
...
$HomoCompInduceHomo \abr ((Homomorphism[\phi, G, *, H, \diamond]) \land (Homomorphism[\theta, H, \diamond, K, \square])) \implies (Homomorphism[\theta \circ \phi, G, *, K, \square])$
\begin{enumerate}
  \lit $FuncComp$ \pipe $Func[\theta \circ \phi, G, K]$
  \lit $(g_1, g_2 \in G) \implies \ldots$
  \begin{enumerate}
    \lit $(Homomorphism[\phi, G, *, H, \diamond]) \land (Homomorphism[\theta, H, \diamond, K, \square])$ \pipe $\theta \circ \phi(g_1 * g_2) = \theta(\phi(g_1 * g_2)) =$ \cont
  \lit \cont $\theta(\phi(g_1) \diamond \phi(g_2)) = \theta(\phi(g_1)) \square \theta(\phi(g_2)) = \theta \circ \phi(g_1) \square \theta \circ \phi(g_2)$ \pipe $\theta \circ \phi(g_1 * g_2) = \theta \circ \phi(g_1) \square \theta \circ \phi(g_2)$
  \end{enumerate}
  \lit $(g_1, g_2 \in G) \implies (\theta \circ \phi(g_1 * g_2) = \theta \circ \phi(g_1) \square \theta \circ \phi(g_2))$ \pipe $\forall_{g_1, g_2 \in G}(\theta \circ \phi(g_1 * g_2) = \theta \circ \phi(g_1) \square \theta \circ \phi(g_2))$
  \lit $(Func[\theta \circ \phi, G, K]) \land (\forall_{g_1, g_2 \in G}(\theta \circ \phi(g_1 * g_2) = \theta \circ \phi(g_1) \square \theta \circ \phi(g_2)))$ \pipe $Homomorphism[\theta \circ \phi, G, *, K, \square]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
\end{array}\right)$
\begin{enumerate}
  \lit $\exists_{g, h}((Generator[g, G, *]) \land (Generator[h, H, \diamond]))$
  \lit TODO $\phi(g^n) = h^n$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 


\section{Kernel and Image Homomorphisms}
$Kernel[ker_\phi, \phi, G, *, H, \diamond] \abr (Homomorphism[\phi, G, *, H, \diamond]) \land (ker_\phi = \{g \in G | \phi(g) = e_H\})$ \\
$Image[im_\phi, \phi, G, *, H, \diamond] \abr (Homomorphism[\phi, G, *, H, \diamond]) \land (im_\phi = \{\phi(g) \in H | g \in G\})$ \\
...
$KerCountsMapSameEl \abr ((Homomorphism[\phi, G, *, H, \diamond]) \land (g \in G)) \implies ((ker_\phi) g = \{x \in G | \phi(x) = \phi(g)\})$
\begin{enumerate}
  ...
  \end{enumerate}
  \lit $(x \in (ker_\phi) g) \implies (\phi(x) = \phi(g))$ \pipe $(ker_\phi) g \subseteq \{x \in G | \phi(x) = \phi(g)\}$
  ...
  \end{enumerate}
  \lit $(\phi(x) = \phi(g)) \implies (x \in (ker_\phi) g)$ \pipe $\{x \in G | \phi(x) = \phi(g)\} \subseteq (ker_\phi) g$
  \lit $((ker_\phi) g \subseteq \{x \in G | \phi(x) = \phi(g)\}) \land (\{x \in G | \phi(x) = \phi(g)\} \subseteq (ker_\phi) g)$ \pipe $(ker_\phi) g = \{x \in G | \phi(x) = \phi(g)\}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
\begin{enumerate}
  \lit $\phi \abr \{\langle a^{-1} g a, C(g) a \rangle \in (C_g \times G : C(g)) | a \in G\}$
...
$CosetMul[\bbin{*}, H, G, *] \abr (Subgroup[H, G, *]) \land (\forall_{H x, H y \in G / H}(H x \bbin{*} H y = \{h_1 x h_2 y | h_1, h_2 \in H\}))$ \\
$SubsetMul[\bbin{\times}, G, *] \abr (Group[G, *]) \land (\forall_{A, B \subseteq G}(A \bbin{\times} B = \{a * b | (a \in A) \land (b \in B)\}))$ \\
...
$SecondIsoThm \abr ((Subgroup[H, G, *]) \land (NormalSubgroup[N, G, *])) \implies (Isomorphic[(H N) / N, \bbin{*}, H / (H \cap N), \bbin{*}])$
\begin{enumerate}
  ...
  \lit 123123
  %\lit $\exists_{\phi}(Isomorphism[\phi, (H N) / N, \bbin{*}, H / (H \cap N), \bbin{*}])$ \pipe $Isomorphic[(H N) / N, \bbin{*}, H / (H \cap N), \bbin{*}]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$ThirdIsoThm \abr \left(\begin{array}{ll}
  ...
  (Isomorphic[G / H, \bbin{*}, (G / K) / (H / K), \bbin{*}])
\end{array}\right)$
\begin{enumerate}
  \lit TODO
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \lit $(a + b = \sum_{i = 1}^{n}(({k_a}_i + {k_b}_i) * s_i)) \land (\langle {k_a}_i + {k_b}_i | i \in \mathbb{N}_{1, n} \rangle \in \mathbb{R}^n)$ \cont
      ...
      \begin{enumerate}
        ...
        \lit $(r * u = \sum_{i = 1}^{n}(r k_i) * s_i)) \land (\langle r k_i \in \mathbb{R} | i \in \mathbb{N}_{1, n} \rangle \in \mathbb{R}^n)$ \pipe $\exists_{M \in \mathbb{R}^n}(r * u = \sum_{i = 1}^{n}(m_i * s_i))$ 
      ...
      \begin{enumerate}
        \lit $K_s \abr \langle \left(\begin{cases} 
          1 & j = i \\
          0 & j \neq i 
        \end{cases}\right) | j \in \mathbb{N}_{1, n} \rangle$ \pipe $(K_s \in \mathbb{R}^n) \land (\sum_{j = 1}^{n}({k_s}_j * s_j) = s_i)$
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \lit $(\forall_{w \in W} \exists!_{v \in V}(w = L(v))) \land (S = \{(w, v) \in W \times V | w = L(v)\})$ \pipe $Function[S, W, V]$
...
$LTMatrix \abr \forall_{L, V, W}\left(\begin{array}{ll}
  ...
  (\forall_{v \in V}(CoordVec[[L(v)]_B, L(v), B, W, +_w, *_w] = \langle [L(a_i)]_B | a_i \in A \rangle * CoordVec[[v]_A, v, A, V, +_v, *_v]))
\end{array}\right)$
\begin{enumerate}
...
$TransitionMatrix \abr \forall_{L, V}\left(\begin{array}{ll}
  ...
  (\forall_{v \in V}(CoordVec[[v]_B, v, B, W, +_w, *_w] = \langle [a]_B | a \in A \rangle * CoordVec[[v]_A, v, A, V, +_v, *_v]))
\end{array}\right)$
\begin{enumerate}
