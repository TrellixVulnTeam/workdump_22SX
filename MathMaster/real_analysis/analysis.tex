...
$AddCom \abr \forall_{A, B \in \mathcal{M}}(A + B = B + A)$ \pipe $\Leftarrow$ $A + B = [a_{i, j} + b_{i, j}] = [b_{i, j} + a_{i, j}] = B + A$ \\
$AddAssoc \abr \forall_{A, B, C \in \mathcal{M}}((A + B) + C = A + (B + C))$ \pipe $\Leftarrow$ $(A + B) + C = [(a_{i, j} + b_{i, j}) + c_{i, j}] = [a_{i, j} + (b_{i, j} + c_{i, j})] = A + (B + C)$ \\
$AddId \abr \forall_{A \in \mathcal{M}} \exists!_{O \in \mathcal{M}}(A + O = A = O + A)$    \phantom{TODO}--- \\ % \pipe $\Leftarrow$ $A + O = [a_{i, j} + 0] = A = [0 + a_{i, j}] = O + A$ \\
$AddInv \abr \forall_{A \in \mathcal{M}} \exists!_{(-A) \in \mathcal{M}}(A + (-A) = O = (-A) + A)$    \phantom{TODO}--- \\ % \pipe $\Leftarrow$ $A + (-A) = [a_{i, j} - a_{i, j}] = O = [-a_{i, j} + a_{i, j}] = (-A) + A$ \\

$MulAssoc \abr \forall_{A, B, C \in \mathcal{M}}((A * B) * C = A * (B * C))$ \pipe $\Leftarrow$
\begin{enumerate}
...
$LeftDist \abr \forall_{A, B, C \in \mathcal{M}}((A + B) * C = A * C + B * C)$ \pipe $\Leftarrow$
\begin{enumerate}
...
$RightDist \abr \forall_{A, B, C \in \mathcal{M}}(C * (A + B) * C = C * A + C * B)$    \phantom{TODO}--- \\
$Scalar1 \abr \forall_{r, s \in \mathbb{R}} \forall_{A \in \mathcal{M}}(r (s A) = (r s) A = s (r A))$    \phantom{TODO}--- \\
$Scalar2 \abr \forall_{r \in \mathbb{R}} \forall_{A, B \in \mathcal{M}}(A (r B) = r (A * B))$    \phantom{TODO}--- \\
$Scalar3 \abr \forall_{r, s \in \mathbb{R}} \forall_{A \in \mathcal{M}}((r + s) A = r A + s A)$    \phantom{TODO}--- \\
$Scalar4 \abr \forall_{r \in \mathbb{R}} \forall_{A, B \in \mathcal{M}}(r (A + B) = r A + r B)$    \phantom{TODO}--- \\

$Trans1 \abr \forall_{A \in \mathcal{M}}(A = (A^T)^T)$ \pipe $\Leftarrow$ $A = [a_{i, j}] = [a_{j, i}]^T = ([a_{i, j}]^T)^T = (A^T)^T$ \\
$Trans2 \abr \forall_{A, B \in \mathcal{M}}((A + B)^T = A^T + B ^T)$    \phantom{TODO}--- \\
$Trans3 \abr \forall_{A, B \in \mathcal{M}}((A * B)^T = B^T * A^T)$ \pipe $\Leftarrow$ 
\begin{enumerate}
...
$Sym(A) \abr A = A^T$ \\
$SkewSym(A) \abr A = -A^T$ \\
$SymGen \abr \forall_{A \in \mathcal{M}}(Sym(A + A^T))$ \pipe $\Leftarrow$ $(A + A^T)^T = A^T + (A^T)^T = A^T + A = A + A^T$ \\
$SkewSymGem \abr \forall_{A \in \mathcal{M}}(SkewSym(A - A^T))$ \pipe $\Leftarrow$ $-(A - A^T)^T = -(A^T - (A^T)^T) = -(A^T - A) = (A - A^T)$ \\
$SymDec \abr \forall_{A \in \mathcal{M}} \exists!_{B : Sym(B)} \exists!_{C : SkewSym(C)}(A = B + C = (1 / 2) * (A + A^T) + (1 / 2) * (A - A^T))$    \phantom{TODO}--- \\

$Square(A, n) \abr Matrix(A, n, n)$ \\
$UpperTriangular(A) \abr (Square(A)) \land (i > \implies a_{i, j} = 0)$ \\
...
$MulId \abr \forall_{A : Square(A, n)}(A * I_n = A = I_n * A)$    \phantom{TODO}--- \\

$Invertible(A) \abr (Square(A, n)) \land (\exists_{A^{-1} \in \mathcal{M}}(A * A^{-1} = I_n = A^{-1} * A))$ \\
$Inv1 \abr \forall_{A : Invertible(A)}(\exists!_{A^{-1} \in \mathcal{M}}(A * A^{-1} = I_n = A^{-1} * A))$    \phantom{TODO}--- \\
$Inv2 \abr \forall_{A : Invertible(A)} \forall_{B : Invertible(B)}((A * B)^{-1} = B^{-1} * A^{-1})$ \pipe $\Leftarrow$
\begin{enumerate}
  \lit $(A * B) * (A * B)^{-1} = I$ \pipe $B * (A * B)^{-1} = A^{-1}$ \pipe $(A * B)^{-1} = B^{-1} * A^{-1}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$Inv3 \abr \forall_{A : Invertible(A)}((A^{-1})^{-1} = A)$    \phantom{TODO}--- \\
...
$RowOp(\phi) \abr (Definition 1.19)$ \\
$RowEquiv(A, B) \abr \exists_{\Phi : |\Phi| \in \mathbb{N}}((\forall_{\phi \in \Phi}(RowOp(\phi))) \land (\Phi(A) = B))$ \\
$RREF(A) \abr (Definition 1.18)$ \\
$GaussJordan \abr \forall_{A} \exists!_{B}(RREF(B) \land RowEquiv(A, B))$    \phantom{TODO}--- \\
$SysAlgebra \abr \forall_{A, B, C, D, X}((RowEquiv([A|B], [C|D])) \implies (Sys(A, X, B) \iff Sys(C, X, D)))$    \phantom{TODO}--- \\
$SysAlgebraCorollary \abr \forall_{A, B, X}((RowEquiv(A, B)) \implies (Sys(A, X, O) \iff Sys(B, X, O)))$    \phantom{TODO}--- \\
$TrivSolLemma \abr (m < n) \implies (\exists_{X}(\lnot TrivSol(X, A, O)))$ \pipe $(\forall_{X}(TrivSol(X, A, O)) \implies (m \geq n)$ \pipe $\Leftarrow$
\begin{enumerate}
  \lit $B = GaussJordan(A)$ \pipe $RowEquiv(A, B)$ \pipe $Sys(A, X, O) \iff Sys(B, X, O)$
  \lit $((m < n) \land Sys(B, X, O)) \implies \text{X has at least n - m free variables}$
...
$Sys(A, X, B) \abr (A * X = B) \land (Matrix(A, m, n)) \land (Matrix(X, n, 1)) \land (Matrix(B, m, 1))$ \\
$Sol(X, A, B) \abr Sys(A, X, B)$ \\
$TrivSol(X, A, B) \abr (Sol(X, A, B)) \land (x_{i, 1} = 0)$ \\

$Elem(A, n) \abr RowEquiv(A, I_n)$ \\
$RowOpAsElem \abr \forall_{\phi : RowOp(\phi)}((E = \phi(I)) \implies (\phi(A) = E * A))$ \\
$RowEquiv2 \abr RowEquiv(A, B) \iff \exists_{E : |E| \in \mathbb{N}}(A = \Pi_{E_i \in E}(E_i) * B)$ \\
$(Elem_i(A)) \implies (Invertible(A) \land Elem_i(A^{-1}))$ \\
$(RowEquiv(A, B)) \iff (B = \Pi(E_i) * A)$ \\
$(RowEquiv(I, B)) \iff (B = \Pi(E_i))$ \\

 FUCKING BRAIN FART PIECE OF SHIT FUCKING FUCKER
 FUCKING BRAIN FART PIECE OF SHIT FUCKING FUCKER
 FUCKING BRAIN FART PIECE OF SHIT FUCKING FUCKER
 FUCKING BRAIN FART PIECE OF SHIT FUCKING FUCKER
 FUCKING BRAIN FART PIECE OF SHIT FUCKING FUCKER
...
$(\forall_{X}(TrivSol(X, A, O)) \implies (RowEquiv(A, I))$ \pipe $\Leftarrow$
\begin{enumerate}
...
$(Invertible(A)) \iff (A = \Pi(E_i))$ \pipe $\Leftarrow$
\begin{enumerate}
  ...
  \end{enumerate}
  \lit $(Invertible(A)) \iff (A = \Pi(E_i))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$(Invertible(A)) \iff (RowEquiv(I, A)))$ \pipe $\Leftarrow$ $(RowEquiv(I, B) \iff B = \Pi(E_i)) \land ((Invertible(A)) \iff (A = \Pi(E_i)))$ \\

$(\forall_{X}(TrivSol(X, A, O))) \implies (Invertible(A))$ \pipe $\Leftarrow$
\begin{enumerate}
   \lit $((\forall_{X}(TrivSol(X, A, O)) \implies (RowEquiv(A, I))) \land ((Invertible(A)) \iff (RowEquiv(I, A))))$
 \end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$(Invertible(A)) \implies (\forall_{X}(TrivSol(X, A, O)))$ \pipe $\Leftarrow$ $A X = O$ \pipe $X = A^{-1} O = O$ \pipe $\forall_{X}(TrivSol(X, A, O)$ \\
