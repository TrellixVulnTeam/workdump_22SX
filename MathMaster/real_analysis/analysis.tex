...
$RankNullity \abr \forall_{L, V, W}((LinTrans[L, V, +_v, *_v, W, +_w, *_w]) \implies (Dim[V] = Dim[ker_L] + Dim[rng_L]))$
\begin{enumerate}
...
\begin{enumerate}
  ...
  \lit $(RankNullity) \land (Dim[ker_T] = 0)$ \pipe $Dim[V] = Dim[ker_T] + Dim[rng_T] = Dim[rng_T]$ \pipe $Dim[V] = Dim[rng_T]$
...
\end{array}\right)$
\begin{enumerate}
  \lit $RankNullity$ \pipe $Dim[V] = Dim[ker_T] + Dim[rng_T]$
...
\begin{enumerate}
  \lit $(RankNullity) \land (Dim[W] \geq Dim[rng_T])$ \pipe $Dim[ker_T] = Dim[V] - Dim[rng_T] \geq Dim[V] - Dim[W] > 0$ \pipe $Dim[ker_T] \neq 0$
...
\begin{enumerate}
  \lit $RankNullity$ \pipe $Dim[rng_T] = Dim[V] - Dim[ker_T] \leq Dim[V] < Dim[W]$
