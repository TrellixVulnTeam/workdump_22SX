...
\begin{comment}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      \lit $(s \in A + B) \implies \ldots$
      \begin{enumerate}
        \lit $s \in A + B$ \pipe $(\exists_{\langle a, b \rangle \in A \times B}(s = a + b))$
...
\begin{comment}
  ...
  \begin{enumerate}
    ...
    \lit $(FinDimBasis) \land (FinDim[U, +, *])$ \pipe $\exists_{B}(Basis[B, U, +, *])$ \pipe $LinInd[B, U, +, *]$ \pipe $LinInd[B, V, +, *]$ TODO
...
$Ran[ran_L, L, V, +_v, *_v, W, +_w, *_w] \abr (LinTrans[L, V, +_v, *_v, W, +_w, *_w]) \land (ran_L = \{\beta \in W | \exists_{\alpha \in V}(\beta = L(\alpha))\})$ \\

$RangeSub \abr \forall_{L, V, W}((Ran[ran_L, L, V, +_v, *_v, W, +_w, *_w]) \implies (Subspace[ran_L, W, +_w, *_w]))$
\begin{enumerate}
  \lit $ZeroMapsToZero$ \pipe $O_w = L(O_v)$ \pipe $\exists_{\alpha \in V}(O_w = L(\alpha))$ \pipe $O_w \in ran_L$ \pipe $\emptyset \neq ran_L$ \pipe $\emptyset \neq ran_L \subseteq W$
  \lit $(\alpha, \beta \in ran_L) \implies \ldots$
  \begin{enumerate}
    \lit $(\exists_{u \in V}(\alpha = L(u))) \land (\exists_{v \in V}(\beta = L(v)))$
    \lit $\alpha + \beta = L(u) + L(v) = L(u + v)$ \pipe $\exists_{w \in V}(\alpha + \beta = L(w))$ \pipe $\alpha + \beta \in ran_L$
  \end{enumerate}
  \lit $(\alpha, \beta \in ran_L) \implies (\alpha + \beta \in ran_L)$ \pipe $\forall_{\alpha, \beta \in ran_L}(\alpha + \beta \in ran_L)$
  \lit $((r \in \mathbb{R}) \land (\alpha \in ran_L)) \implies \ldots$
  \begin{enumerate}
    \lit $\exists_{v \in V}(\alpha = L(v))$ \pipe $L(r * v) = r * L(v) = r * \alpha$ \pipe $\exists_{w \in V}(r * \alpha = L(w))$ \pipe $r * \alpha \in ran_L$
  \end{enumerate}
  \lit $((r \in \mathbb{R}) \land (\alpha \in ran_L)) \implies (r * \alpha \in ran_L)$ \pipe $\forall_{r \in \mathbb{R}} \forall_{\alpha \in ran_L}(r * \alpha \in ran_L)$
  \lit $(SubspaceEquiv) \land (\emptyset \neq ran_L \subseteq W) \land (\forall_{\alpha, \beta \in ran_L}(\alpha + \beta \in ran_L)) \land (\forall_{r \in \mathbb{R}} \forall_{\alpha \in ran_L}(r * \alpha \in ran_L))$ \pipe $Subspace[ran_L, W, +_w, *_w]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$RankKer \abr \forall_{L, V, W}((LinTrans[L, V, +_v, *_v, W, +_w, *_w]) \implies (Dim[V] = Dim[ker_L] + Dim[ran_L]))$
\begin{enumerate}
  \lit $(\exists_{U}(Basis[U, ker_L, +_v, *_v])) \land (m \abr |U|)$
  \lit $p63$ 
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

https://math.stackexchange.com/questions/3576488/is-a-linearly-independent-set-in-a-subspace-also-a-linearly-independent-set-in
