...
\begin{enumerate}
  \lit $(g \in Z(g)) \iff (\forall_{x \in G}(x * g = g * x)) \iff (\forall_{x \in G}(x * g * x^{-1} = g)) \iff (\forall_{f \in G}((f \sim^* g) \iff (f = g))) \iff (C_g = \{g\})$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
\begin{enumerate}
  \lit $CosetCardinality$ \pipe $(NormalSubgroup[H, G, *]) \iff (\forall_{h \in H} \forall_{g \in G}(g^{-1} h g \in H)) \iff (\forall_{h \in H} \forall_{g \in G}(h g \in g H)) \iff$ \cont
  \lit \cont $(\forall_{g \in G}(H g \subseteq g H)) \iff (\forall_{g \in G}(H g = g H))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$SimMatrixEquiv \abr (SimMatrix[A, B]) \iff (([L(a)]_T = A * [a]_S) \land ([L(a)]_{T'} = B * [a]_{S'}))$ ??? \\
