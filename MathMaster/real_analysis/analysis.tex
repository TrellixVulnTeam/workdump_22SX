...
$Comp[g \circ f, f, g, X, Y, Z] \abr (Func[f, X, Y]) \land (Func[g, Y, Z]) \land (Func[g \circ f, X, Z]) \land (g \circ f = \{\langle x, g(f(x)) \rangle \in X \times Z | x \in X\})$ \\
...
$ExpModOrder \abr \forall_{G, g, n, s, t}((Group[G, *]) \land (g \in G) \land (OrderEl[n, g, G, *]) \implies ((g^s = g^t) \iff (s \equiv t (mod \phantom{.} n))))$
\begin{enumerate}
  \lit TODO
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$ExpMapsExp \abr (Homomorphism[\phi, G, *, H, \diamond]) \implies (\phi(g^n) = \phi(g)^n)$
\begin{enumerate}
