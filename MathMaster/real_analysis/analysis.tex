...
$+(A, B) \abr (A + B = [a_{i, j} + b_{i, j}]_{m \times n}) \land (Matrix(A, m, n)) \land (Matrix(B, m, n))$ \\
$*(r, A) \abr (r * A = [r a_{i, j}]_{m \times n}) \land (Matrix(A, m, n))$ \\
$*(A, B) \abr (A * B = \left[\sum_{k = 1}^{p}(a_{i, k} b_{k, j})\right]_{m \times n}) \land (Matrix(A, m, p)) \land (Matrix(B, p, n))$ \\
$\phantom{ }^T(A) \abr (A^T = [a_{j, i}]_{n \times m}) \land (Matrix(A, m, n))$ \\

$AddCom \abr A + B = B + A$ \pipe $\leftarrow$ $A + B = [a_{i, j} + b_{i, j}] = [b_{i, j} + a_{i, j}] = B + A$ \\
$AddAssoc \abr (A + B) + C = A + (B + C)$ \pipe $\leftarrow$ $(A + B) + C = [(a_{i, j} + b_{i, j}) + c_{i, j}] = [a_{i, j} + (b_{i, j} + c_{i, j})] = A + (B + C)$ \\
$AddId \abr A + O = A = O + A$ \pipe $\leftarrow$ $A + O = [a_{i, j} + 0] = A = [0 + a_{i, j}] = O + A$ \\
$AddInv \abr A + (-A) = O = (-A) + A$ \pipe $\leftarrow$ $A + (-A) = [a_{i, j} - a_{i, j}] = O = [-a_{i, j} + a_{i, j}] = (-A) + A$ \\

$MulAssoc \abr (A * B) * C = A * (B * C)$ \pipe $\leftarrow$
\begin{enumerate}
...
$LeftDist \abr (A + B) * C = A * C + B * C$ \pipe $\leftarrow$
\begin{enumerate}
...
$RightDist \abr C * (A + B) * C = C * A + C * B$    \phantom{TODO}--- \\
$Scalar1 \abr r (s A) = (r s) A = s (r A)$    \phantom{TODO}--- \\
$Scalar2 \abr A (r b) = r (A B)$    \phantom{TODO}--- \\
$Scalar3 \abr (r + s) A = r A + r S$    \phantom{TODO}--- \\
$Scalar4 \abr r (A + B) = r A + r B$    \phantom{TODO}--- \\

$Trans1 \abr A = (A^T)^T$ \pipe $\leftarrow$ $A = [a_{i, j}] = [a_{j, i}]^T = ([a_{i, j}]^T)^T = (A^T)^T$ \\
$Trans2 \abr (A + B)^T = A^T + B ^T$    \phantom{TODO}--- \\
$Trans3 \abr (A * B)^T = B^T * A^T$ \pipe $\leftarrow$ $(A * B)^T = \left[\sum_{k = 1}^{p}(a_{i, k} b_{k, j})\right]^T = \left[\sum_{k = 1}^{p}(a_{j, k} b_{k, i})\right] = \left[\sum_{k = 1}^{p}(b_{k, i} a_{j, k})\right] =  \left[\sum_{k = 1}^{p}(b^T_{i, k} a^T_{k, j})\right] = B^T * A^T$ \\

$Sym(A) \abr A = A^T$ ; $SkewSym(A) \abr A = -A^T$ \\
$SymGen \abr Sym(A + A^T)$ \pipe $\leftarrow$ $(A + A^T)^T = A^T + (A^T)^T = A^T + A = A + A^T$ \\
$SkewSymGem \abr SkewSym(A - A^T)$ \pipe $\leftarrow$ $-(A - A^T)^T = -(A^T - (A^T)^T) = -(A^T - A) = (A - A^T)$ \\
$SymDec \abr A = (1 / 2) * (A + A^T) + (1 / 2) * (A - A^T)$    \phantom{TODO}--- \\

$Square(A) \abr Matrix(A, n, n)$ \\
$Diagonal(A) \abr (Square(A)) \land (i \neq j \implies a_{i, j} = 0)$ \\
$Scalar(A) \abr (Diagonal(A)) \land (a_{i, i} = k)$ \\
$I \abr (Scalar(I)) \land (i_{d, d} = 1)$ \\
$MulId \abr A * I = A = I * A$    \phantom{TODO}--- \\

$UpperTriangular(A) \abr (Square(A)) \land (i > \implies a_{i, j} = 0)$ \\
$LowerTriangular(A) \abr (Square(A)) \land (i < j \implies a_{i, j} = 0)$ \\

$Invertible(A) \abr \exists_{A^{-1}}(A * A^{-1} = I = A^{-1} * A)$ \\
$UniqueInverse \abr ((A * B_1 = I) \land (A * B_2 = I)) \implies B_1 = B_2$    \phantom{TODO}--- \\
$Inv1 \abr (Invertible(A) \land Invertible(B)) \implies (A * B)^{-1} = B^{-1} * A^{-1}$ \pipe $\leftarrow$
\begin{enumerate}
  \lit $(A * B) * (A * B)^{-1} = I$ \pipe $B * (A * B)^{-1} = A^{-1}$ \pipe $(A * B)^{-1} = B^{-1} * A^{-1}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
$Inv2 \abr (Invertible(A)) \implies ((Invertible(A^{-1}) \land (A = (A^{-1})^{-1}))$    \phantom{TODO}--- \\
$InvTrans \abr (Invertible(A)) \implies ((Invertible(A^T) \land ((A^T)^{-1} = (A^{-1})^T))$ \pipe $\leftarrow$
\begin{enumerate}
...
$TrivSol(X, A, B) \abr (Sol(X, A, B)) \land (a_{i. j} = 0)$ \\

$RREF(A) \abr (Definition 1.18)$ \\
$ElementaryRowOperation(\phi) \abr (Definition 1.19)$ \\
$RowEquiv(A, B) \abr \exists_{\Phi}(\forall_{\phi \in \Phi}(ElementaryRowOperation(\phi)) \land |\Phi| \in \mathbb{N} \land \Phi(A) = B)$ \\

$(A \neq O) \implies (\exists_{B}(RREF(B) \land RowEquiv(A, B)))$ (By Gauss-Jordan Elimination)    \phantom{TODO}--- \\
$(Sys(A, X_1, B) \land Sys(C, X_2, D) \land RowEquiv([A|B], [C|D])) \implies (X_1 = X_2)$ (By algebra on systems of equations)    \phantom{TODO}--- \\
$(RowEquiv(A, B)) \implies (Sys(A, X_1, O) \land Sys(B, X_1, O))$ (Corollary)    \phantom{TODO}--- \\
$(m < n) \implies (\exists_{X}(\lnot TrivSol(X, A, O)))$ \pipe $(\forall_{X}(TrivSol(X, A, O)) \implies (m \geq n)$ \pipe $\leftarrow$
\begin{enumerate}
...
$Elem(A) \abr RowEquiv(A, I)$ \\
$(Elem_i(A)) \implies (Invertible(A) \land Elem_i(A^{-1}))$ \\
$(ElementaryRowOperation(\phi) \land (B = \phi(A))) \implies (Elem(E) \land B = E * A) $ \\
