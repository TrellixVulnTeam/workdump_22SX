...
$RowEquivHomoSys \abr \forall_{A, C, X \in \mathcal{M}}((RowEquiv[A, C]) \implies ((Sol[X, A, O]) \iff (Sol[X, C, O])))$
\begin{enumerate}
  \lit Set $B = D = O$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$RREFInv \abr \forall_{A \in \mathcal{M}}((Invertible[A]) \iff (RowEquiv[A, I_n]))$
\begin{enumerate}
