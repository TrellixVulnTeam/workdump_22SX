...
$SubspaceEquiv \abr \forall_{V : VectorSpace[V, +, *]} \forall_{S : \emptyset \neq S \subseteq V}((Subspace[S, V, +, *]) \iff ((\forall_{r, s \in S}(r + s \in S)) \land (\forall_{\alpha \in \mathbb{R}} \forall_{s \in S}(\alpha * s \in S))))$
\begin{enumerate}
...
$LinearCombination[c, U, V, +, *] \abr (VectorSpace[V, +, *]) \land (U \subseteq V) \land (n = |U| \in \mathbb{N}) \land (\exists_{\Gamma \in \mathbb{R}^{n}}(c = \sum_{i = 1}^{n}(\gamma_i * u_i)))$ \\
$LinearSpan[S, U, V, +, *] \abr \left(\begin{array}{ll}
  (VectorSpace[V, +, *]) \hfill \land \hfill (U \subseteq V) \hfill \land \hfill (|U| \in \mathbb{N}) \hfill \land \hfill ((U = \emptyset) \implies (S = \{O\})) \hfill \land \hfill \\
...
$SubIndependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  ...
  ((LinearlyIndependent[B, V, +, *]) \implies (LinearlyIndependent[A, V, +, *]))
\end{array}\right)$
\begin{enumerate}
  ...
  \end{enumerate}
  \lit $(\sum(\gamma_i * a_i = O)) \implies (\Gamma = \{0\}^{|A|})$ \pipe $LinearlyIndependent[A, V, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SuperDependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  ((VectorSpace[V, +, *]) \land (A \subseteq B \subseteq V) \land (|B| \in \mathbb{N})) \hfill \implies \hfill \\
  ((\lnot LinearlyIndependent[A, V, +, *]) \implies (\lnot LinearlyIndependent[B, V, +, *]))
\end{array}\right)$
\begin{enumerate}
  \lit $U \abr B \setminus A$
  \lit $\sum(0 * u_i) = O$
...
$LinearlyIndependentEquiv \abr \forall_{U, V}((LinearlyIndependent[U, V, +, *]) \iff (\forall_{j \in U}(\lnot LinearCombination[j, U \setminus \{j\}, +, *])))$
\begin{enumerate}
  \lit $(\lnot LinearlyIndependent[U, V, +, *]) \implies \ldots$
  \begin{enumerate}
    \lit 123123
   \end{enumerate}
  \lit $(\lnot LinearlyIndependent[U, V, +, *]) \implies (\exists_{j \in U}(LinearCombination[j, U \setminus \{j\}, +, *]))$ \pipe $(\forall_{j \in U}(\lnot LinearCombination[j, U \setminus \{j\}, +, *])) \implies (LinearlyIndependent[U, V, +, *])$
  \lit $(\exists_{j \in U}(LinearCombination[j, U \setminus \{j\}, +, *])) \implies \ldots$
  \begin{enumerate}
    \lit 123123
  \end{enumerate}
  \lit $(\exists_{j \in U}(LinearCombination[j, U \setminus \{j\}, +, *])) \implies (\lnot LinearlyIndependent[U, V, +, *])$ \pipe $(LinearlyIndependent[U, V, +, *]) \implies (\forall_{j \in U}(\lnot LinearCombination[j, U \setminus \{j\}, +, *]))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
