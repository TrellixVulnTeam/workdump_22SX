...
\begin{enumerate}
  \lit $(CosetInduceEqRel) \land (EqRelInducesPartition)$ \pipe $\exists_{k \in \mathbb{N}}(n = m k)$ \pipe $Divides[m, n]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $x = K_x * g$ \pipe $\phi(x) = \phi(K_x * g) = \phi(K_x) \diamond \phi(g) = e_H \diamond \phi(g) = \phi(g)$ \pipe $\phi(x) = \phi(g)$
  \end{enumerate}
  \lit $(x \in (ker_\phi) g) \implies (\phi(x) = \phi(g))$
  \lit $(\phi(x) = \phi(g)) \implies \ldots$
  \begin{enumerate}
    \lit TODO: other direction
  \end{enumerate}
