...
$LinComb[c, U, K, V, +, *] \abr (VectorSpace[V, +, *]) \land (U \subseteq V) \land (K \subseteq \mathbb{R}) \land (n = |U| = |K| \in \mathbb{N}) \land (c = \sum_{i = 1}^{n}(k_i * u_i))$ \\
$LinSpan[S', S, V, +, *] \abr \left(\begin{array}{ll}
  (VectorSpace[V, +, *]) \land (S \subseteq V) \land ((S = \emptyset) \implies (S' = \{O\})) \hfill \land \hfill \\
  ((S \neq \emptyset) \implies (S' = \{c \in V | \exists_{K \subseteq \mathbb{R}}(LinComb[c, S, K, V, +, *])\}))
\end{array}\right)$ \\
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $u \in S'$ \pipe $\exists_{K \subseteq \mathbb{R}}(LinComb[u, S, K, V, +, *])$ \pipe $\exists_{K \subseteq \mathbb{R}}(u = \sum_{i = 1}^{n}(k_i * s_i))$
    \lit $v \in S'$ \pipe $\exists_{L \subseteq \mathbb{R}}(LinComb[v, S, L, V, +, *])$ \pipe $\exists_{L \subseteq \mathbb{R}}(v = \sum_{i = 1}^{n}(l_i * s_i))$
    ...
    \lit $\exists_{M \subseteq \mathbb{R}}(u + v = \sum_{i = 1}^{n}(m_i * s_i))$ \pipe $\exists_{M \subseteq \mathbb{R}}(LinComb[u + v, S, M, V, +, *])$ \pipe $u + v \in S'$
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \lit $\exists_{M \subseteq \mathbb{R}}(r * u = \sum_{i = 1}^{n}(m_i * s_i))$ \pipe $\exists_{M \subseteq \mathbb{R}}(LinComb[r * u, S, M, V, +, *])$ \pipe $r * u \in S'$
  \end{enumerate}
...
$SmallestSubspaceContaining \abr \forall_{T, S', S, V}(((LinSpan[S', S, V, +, *]) \land (Subspace[T, V, +, *]) \land (S \subseteq T)) \implies (S' \subseteq T))$
\begin{enumerate}
  \lit $LSSubspaceContaining$ \pipe $(Subspace[S', V, +, *]) \land (S \subseteq S')$
  \lit $VectorSpace[S', +, *]$

  \lit $(W \subseteq V) \implies (LinSpan[W, W, V, +, *] \iff Subspace[W, V, +, *])$
  \lit $((W \subseteq V) \land (Subspace[W, V, +, *]) \land (S \subseteq W) \land (LinSpan[S', S, W, +, *])) \implies (Subspace[S', W, +, *])$

  \lit $\forall_{T \subset S'}((\lnot Subspace[T, V, +, *]) \lor (S \not \subseteq T))$
  \lit $\lnot \exists_{T \subset S'}((Subspace[T, V, +, *]) \land (S \subseteq T))$
...
$LinInd[U, V, +, *] \abr \left(\begin{array}{ll}
  ...
  (\forall_{\Gamma \in \mathbb{R}^n}((\sum_{i = 1}^{n}(\gamma_i * u_i) = O) \implies (\Gamma = \{0\}^n)))
\end{array}\right)$ \\
...
\begin{enumerate}
  \lit $(r * \alpha = O) \iff ((r = 0) \lor (\alpha \neq O))$
  \lit $\alpha \neq O$ \pipe $r = 0$
  \lit $\forall_{r \in \mathbb{R}^1}((r * \alpha = O) \implies (r = 0))$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SubIndependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  ((VectorSpace[V, +, *]) \land (A \subseteq B \subseteq V) \land (|B| \in \mathbb{N})) \hfill \implies \hfill \\
  ((LinInd[B, V, +, *]) \implies (LinInd[A, V, +, *]))
\end{array}\right)$
\begin{enumerate}
  \lit $U \abr B \setminus A$
  \lit $(\sum(\gamma_i * a_i = O)) \implies \ldots$
  \begin{enumerate}
    \lit $\sum(0 * u_i) = O$
  ...
  \end{enumerate}
  \lit $(\sum(\gamma_i * a_i = O)) \implies (\Gamma = \{0\}^{|A|})$ \pipe $LinInd[A, V, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SuperDependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  ((VectorSpace[V, +, *]) \land (A \subseteq B \subseteq V) \land (|B| \in \mathbb{N})) \hfill \implies \hfill \\
  ((\lnot LinInd[A, V, +, *]) \implies (\lnot LinInd[B, V, +, *]))
\end{array}\right)$
\begin{enumerate}
  \lit $U \abr B \setminus A$
  \lit $\sum(0 * u_i) = O$
  \lit $(\exists_{\Gamma \in \mathbb{R}^|A|}((\sum(\gamma_i * a_i) = O) \land (\Gamma \neq \{0\}^{|A|})))$
  \lit $(\sum(\gamma_i * b_i) = \sum(\gamma_i * a_i) + O = O + O = O) \land (\Gamma' = \Gamma \times \{0\}^{|U|} \neq \{0\}^|B|)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$BasisCardProp \abr \forall_{S, T, V}(((Basis[S, V, +, *]) \land (Basis[T, V, +, *])) \implies (|T| = |S|))$
\begin{enumerate}
