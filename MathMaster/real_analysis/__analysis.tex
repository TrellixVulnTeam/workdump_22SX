...
$Center[Z(G), G, *] \abr (Group[G, *]) \land \pr{Z(G) = \cap_{g \in G}\pr{C(g)}}$ \\
...
$NormalSubgroup[H, G, *] \abr (Subgroup[H, G, *]) \land \pr{\forall_{g \in G}(g H = H g)}$ \\
...
\begin{enumerate}
  ...
  \end{enumerate}
  \lit $(g \in G) \implies \pr{g Z(G) = Z(G) g}$ \pipe $\forall_{g \in G}\pr{g Z(G) = Z(G) g}$
  \lit $\pr{Subgroup[Z(G), G, *]} \land \pr{\forall_{g \in G}\pr{g Z(G) = Z(G) g}}$ \pipe $NormalSubgroup[Z(G), G, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
