...
$O_{m, n} \abr \pr{Matrix(O, m, n)} \land (a_{i, j} = 0)$ \\
$Square(A, n) \abr Matrix(A, n, n)$ \\
$UpperTriangular(A) \abr \pr{Square(A)} \land (i > j \implies a_{i, j} = 0)$ \\
...
$SymGen \abr \forall_{A \in \mathcal{M}}\pr{Sym(A + A^T)}$
\begin{enumerate}
  \lit $(A + A^T)^T = A^T + (A^T)^T = A^T + A = A + A^T$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SkewSymGen \abr \forall_{A \in \mathcal{M}}\pr{SkewSym(A - A^T)}$
\begin{enumerate}
...
$Sys(A, B) \abr \pr{Matrix(A, m, n)} \land \pr{Matrix(B, m, 1)}$ \\
$Sol(X, A, B) \abr \pr{Sys(A, B)} \land \pr{Matrix(X, n, 1)} \land (A * X = B)$ \\
$ConsistentSys(A, B) \abr \pr{Sys(A, B)} \land \exists_{X}\pr{Sol(X, A, B)}$ \\
$TrivSol(X, A) \abr \pr{Sol(X, A, O)} \land (X = O)$ \\
$NonTrivSol(X, A) \abr \pr{Sol(X, A, O)} \land (X \neq O)$ \\

$HomoSysProps \abr \pr{Sys(A, O)} \implies \ldots$
\begin{enumerate}
  \lit $u_0 \abr O$ ; $u_1 \abr choice(\{X \in \mathcal{M} | X \neq O\})$ ; $k \abr choice(\mathbb{R})$
  \lit $TrivSol(u_0, A)$
  \lit $\pr{NonTrivSol(u_1, A)} \implies \pr{Sol(u_1 + k u_0)}$
  \lit $\pr{TrivSol(\overrightarrow{X}, A)} \implies \pr{TrivSol\pr{LC(\overrightarrow{X}), A}}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

TODO: Skip RowOp and go straight to ElemMatrices, Remark RowOp as ``working definition ONLY!''
$RowOp(\phi, A) \abr \pr{\phi \equiv RowSwap(A)} \lor \pr{\phi \equiv RowScale_*(A)} \lor \pr{\phi \equiv RowCombine_*(A)}$ \\
$RowOps(\langle \Phi \rangle_1^n, A) \abr \forall_{i < n}\pr{\pr{(i = 1) \implies RowOp(\phi_i, A)} \land \pr{(i > 1) \implies RowOps(\langle \Phi \rangle_i^{n - 1}), \phi_n(A)}}$ \\
$RowEquiv(A, B) \abr \exists_{\langle \Phi \rangle_1^n}\pr{\pr{RowOps(\langle \Phi \rangle_1^n, A)} \land \pr{B = \langle \Phi \rangle_1^n(A)}}$ \\
$ElemMat(E) \abr E = \exists_{\phi}\pr{\pr{RowOp(\phi, I_n)} \land \pr{E = \phi(I_n)}}$ \\
$ElemMats(\langle F \rangle) \abr \forall_{E \in \langle F \rangle}\pr{ElemMat(E)}$ \\

$ElemMatInv \abr \forall_{E} \exists!_{E^{-1}}\pr{\pr{ElemMat(E)} \implies (E * E^{-1} = I_n = E^{-1} * E)}$
\begin{enumerate}
  \lit $E-RowSwap(E) \implies TODO$ ; $E-RowScale_*(E) \implies TODO$ ; $E-RowCombine_*(E) \implies TODO$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$RowEquivElemProd \abr \forall_{A, B \in \mathcal{M}}\pr{RowEquiv(A, B) \implies \exists_{\langle F \rangle}\pr{ElemMats(\langle F \rangle)} \land (B = \langle F \rangle * A)}$
\begin{enumerate}
  \lit $B = \langle \Phi \rangle(A)$
  \lit $\langle F \rangle = \langle E | (\phi \in \Phi) \land \pr{E = \phi(I_n)} \rangle$
  \lit $B = \langle \Phi \rangle(A) = \langle F \rangle * A$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$RowEquivSys \abr \forall_{A, B, C, D \in \mathcal{M}} \forall_{\langle F \rangle | ElemMats(\langle F \rangle)}\pr{\pr{Sys(A, B)} \land \pr{Sys(C, D)} \land ()}$
