...
$Surjective(f, X, Y) \abr Function(f, X, Y) \land \forall_{y \in Y} \exists_{x \in X}\pr{y_0 = f(x)}$ \\ % or Onto
...
$ZeroVectorUniq \abr \forall_{u, v \in V}\pr{(v + u = v) \implies (u = O)}$
\begin{enumerate}
  \lit $(v + u = v)$ \pipe $-v + v + u = -v + v$ \pipe $u = O$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$ZeroVectorGen \abr \forall_{\alpha \in \mathbb{R}} \forall_{u \in V}\pr{(\alpha * u = O) \iff \pr{(u = O) \lor (\alpha = 0)}}$
\begin{enumerate}
  ...
  \lit $\pr{(u = O) \implies (\alpha * u = O)} \land \pr{(\alpha = 0) \implies (\alpha * u = O)}$ \pipe $\pr{(u = O) \lor (\alpha = 0)} \implies (\alpha * u = O))$
  ...
  \end{enumerate}
  \lit $(\alpha * u = O) \implies \pr{(\alpha = 0) \lor (u = O)}$
  \lit $(\alpha * u = O) \iff \pr{(\alpha = 0) \lor (u = O)}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$NegVectorGen \abr \forall_{u \in V}\pr{(-1) * u = -u}$
\begin{enumerate}
  \lit $O = 0 * u = \pr{1 + (-1)} * u = (1 * u) + \pr{(-1) * u} = u + \pr{(-1) * u}$ \pipe $O = u + \pr{(-1) * u}$ \pipe $-u = (-1) * u$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$SubspaceEquiv \abr \forall_{V, S}\left(\begin{array}{ll}
  \pr{(VectorSpace[V, +, *]) \land (\emptyset \neq S \subseteq V)} \hfill \implies \hfill \\
  \pr{(Subspace[S, V, +, *]) \iff \pr{\pr{\forall_{r, s \in S}(r + s \in S)} \land \pr{\forall_{\alpha \in \mathbb{R}} \forall_{s \in S}(\alpha * s \in S)}}}
\end{array}\right)$
\begin{enumerate}
  ...
  \end{enumerate}
  \lit $(Subspace[S, V, +, *]) \implies \pr{\pr{\forall_{r, s \in S}(r + s \in S)} \land \pr{\forall_{\alpha \in \mathbb{R}} \forall_{s \in S}(\alpha * s \in S)}}$
  \lit $\pr{\pr{\forall_{r, s \in S}(r + s \in S)} \land \pr{\forall_{\alpha \in \mathbb{R}} \forall_{s \in S}(\alpha * s \in S)}} \implies \ldots$
  \begin{enumerate}
    \lit $\pr{(\alpha ,\beta \in \mathbb{R}) \land (u, v, w \in S)} \implies \ldots$
    \begin{enumerate}
...
$LinComb[c, U, K, V, +, *] \abr (VectorSpace[V, +, *]) \land (U \subseteq V) \land (K \subseteq \mathbb{R}) \land (n = |U| = |K| \in \mathbb{N}) \land \pr{c = \sum_{i = 1}^{n}(k_i * u_i)}$ \\
$LinSpan[S', S, V, +, *] \abr \left(\begin{array}{ll}
  (VectorSpace[V, +, *]) \land (S \subseteq V) \land \pr{(S = \emptyset) \implies (S' = \{O\})} \hfill \land \hfill \\
  \pr{(S \neq \emptyset) \implies \pr{S' = \{c \in V | \exists_{K \subseteq \mathbb{R}}(LinComb[c, S, K, V, +, *])\}}}
\end{array}\right)$ \\
$Spans[S, V, +, *] \abr LinSpan[V, S, V, +, *]$ \\

$LSSubspaceContaining \abr \forall_{S', S, V}\pr{(LinSpan[S', S, V, +, *]) \implies \pr{(Subspace[S', V, +, *]) \land (S \subseteq S')}}$
\begin{enumerate}
  \lit $LinSpan[S', S, V, +, *]$ \pipe $O \in S'$ \pipe $\emptyset \neq S'$
  \lit $(\emptyset \neq S') \land (S' \subseteq V)$ \pipe $\emptyset \neq S' \subseteq V$
  \lit $(u, v \in S') \implies \ldots$
  \begin{enumerate}
    \lit $u \in S'$ \pipe $\exists_{K \subseteq \mathbb{R}}(LinComb[u, S, K, V, +, *])$ \pipe $\exists_{K \subseteq \mathbb{R}}\pr{u = \sum_{i = 1}^{n}(k_i * s_i)}$
    \lit $v \in S'$ \pipe $\exists_{L \subseteq \mathbb{R}}(LinComb[v, S, L, V, +, *])$ \pipe $\exists_{L \subseteq \mathbb{R}}\pr{v = \sum_{i = 1}^{n}(l_i * s_i)}$
    \lit $u + v = \sum_{i = 1}^{n}(k_i * s_i) + \sum_{i = 1}^{n}(l_i * s_i) = \sum_{i = 1}^{n}\pr{(k_i + l_i) * s_i}$
    \lit $M \abr \{k_i + l_i \in \mathbb{R} | (1 \leq i \leq n) \land (i \in \mathbb{N})\}$ \pipe $M \subseteq \mathbb{R}$
    \lit $\exists_{M \subseteq \mathbb{R}}\pr{u + v = \sum_{i = 1}^{n}(m_i * s_i)}$ \pipe $\exists_{M \subseteq \mathbb{R}}(LinComb[u + v, S, M, V, +, *])$ \pipe $u + v \in S'$
  \end{enumerate}
  \lit $(u, v \in S') \implies (u + v \in S')$ \pipe $\forall_{u, v \in S'}(u + v \in S')$
  \lit $\pr{(r \in \mathbb{R}) \land (u \in S')} \implies \ldots$
  \begin{enumerate}
    \lit $u \in S'$ \pipe $\exists_{K \subseteq \mathbb{R}}(LinComb[u, S, K, V, +, *])$ \pipe $\exists_{K \subseteq \mathbb{R}}\pr{u = \sum_{i = 1}^{n}(k_i * s_i)}$
    \lit $r * u = r * \sum_{i = 1}^{n}(k_i * s_i) = \sum_{i = 1}^{n}\pr{r * (k_i * s_i)} = \sum_{i = 1}^{n}(r k_i) * s_i)$
    \lit $M \abr \{r k_i \in \mathbb{R} | (1 \leq i \leq n) \land (i \in \mathbb{N})\}$ \pipe $M \subseteq \mathbb{R}$
    \lit $\exists_{M \subseteq \mathbb{R}}\pr{r * u = \sum_{i = 1}^{n}(m_i * s_i)}$ \pipe $\exists_{M \subseteq \mathbb{R}}(LinComb[r * u, S, M, V, +, *])$ \pipe $r * u \in S'$
  \end{enumerate}
  ...
  \begin{enumerate}
    \lit $K \abr \{ \left(\begin{cases} 
      1 & i = j \\
      0 & i \neq j 
    \end{cases}\right) | (1 \leq i \leq n) \land (i \in \mathbb{N})\}$ \pipe $K \subseteq \mathbb{R}$
    \lit $\ldots$ \pipe $\exists_{K \subseteq \mathbb{R}}(LinComb[s_j, S, K, V, +, *])$ \pipe $s_j \in S'$
  \end{enumerate}
  \lit $(s_j \in S) \implies (s_j \in S')$ \pipe $\forall_{x \in S}(x \in S')$ \pipe $S \subseteq S'$
  \lit $(Subspace[S', V, +, *]) \land (S \subseteq S')$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$LSSubspaceIdentity \abr (LinSpan[W', W, V, +, *]) \implies \pr{(W' = W) \iff (Subspace[W, V, +, *])}$
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $SubspaceEquiv$ \pipe $\pr{\forall_{r, s \in W}(r + s \in W)} \land \pr{\forall_{\alpha \in \mathbb{R}} \forall_{s \in W}(\alpha * s \in W)}$ \pipe $\forall_{w \in W}(LinComb[w, W, K, V, +, *])$
...
$LSSubspaceSubset \abr \pr{(LinSpan[S', S, V, +, *]) \land (Subspace[W, V, +, *]) \land (S \subseteq W)} \implies (Subspace[S', W, +, *])$
\begin{enumerate}
  \lit $(LinSpan[S', S, V, +, *]) \land (S \subseteq W)$ \pipe $(LinSpan[S', S, W, +, *])$
  \lit $(LSSubspaceContaining) \land (LinSpan[S', S, W, +, *])$ \pipe $Subspace[S', W, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SmallestSubspaceContaining \abr \forall_{W, S', S, V}\pr{\pr{(LinSpan[S', S, V, +, *]) \land (Subspace[W, V, +, *]) \land (S \subseteq W)} \implies (S' \subseteq W)}$
\begin{enumerate}
  \lit $\pr{(Subspace[W, V, +, *]) \land (S \subseteq W)} \implies \ldots$
  \begin{enumerate}
    \lit $LSSubspaceSubset$ \pipe $Subspace[S', W, +, *]$\pipe $S' \subseteq W$
  \end{enumerate}
  \lit $\pr{(Subspace[W, V, +, *]) \land (S \subseteq W)} \implies (S' \subseteq W)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$LinInd[S, V, +, *] \abr   (VectorSpace[V, +, *]) \land (\emptyset \neq S \subseteq V) \land \pr{\forall_{K \subseteq \mathbb{R}}\pr{(LinComb[O, S, K, V, +, *]) \implies  (K = \{0\})}}$

TODO, SAY K is a sequence instead of a set $K \subseteq \mathbb{R}^n$ instead ??

$LinInd[U, V, +, *] \abr \left(\begin{array}{ll}
  (VectorSpace[V, +, *]) \hfill \land \hfill (U \subseteq V) \hfill \land \hfill (n = |U| \in \mathbb{N}) \hfill \land \hfill \\
  \pr{\forall_{\Gamma \in \mathbb{R}^n}\pr{\pr{\sum_{i = 1}^{n}(\gamma_i * u_i) = O} \implies (\Gamma = \{0\}^n)}}
\end{array}\right)$ \\

$SingletonZeroDependent \abr \lnot LinInd[\{O\}, V, +, *]$
\begin{enumerate}
  \lit $(1 * O = O) \land (1 \neq 0)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SingletonNonZeroIndependent \abr (\alpha \neq O) \implies (LinInd[\{\alpha\}, V, +, *])$
\begin{enumerate}
  \lit $(r * \alpha = O) \iff \pr{(r = 0) \lor (\alpha \neq O)}$
  \lit $\alpha \neq O$ \pipe $r = 0$
  \lit $\forall_{r \in \mathbb{R}^1}\pr{(r * \alpha = O) \implies (r = 0)}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SubIndependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  \pr{(VectorSpace[V, +, *]) \land (A \subseteq B \subseteq V) \land (|B| \in \mathbb{N})} \hfill \implies \hfill \\
  \pr{(LinInd[B, V, +, *]) \implies (LinInd[A, V, +, *])}
\end{array}\right)$
\begin{enumerate}
  \lit $U \abr B \setminus A$
  \lit $\pr{\sum(\gamma_i * a_i = O)} \implies \ldots$
  \begin{enumerate}
    \lit $\sum(0 * u_i) = O$
    \lit $\sum(\gamma'_i = b_i) = \sum(\gamma_i * a_i) + \sum(0 * u_i) = O + O = O$
    \lit $LinInd[B, V, +, *]$ \pipe $\Gamma' = \{0\}^{|B|}$
    \lit $\Gamma' = \{0\}^{|B|} = \Gamma \times \{0\}^{|U|}$ \pipe $\Gamma = \{0\}^{|A|}$
  \end{enumerate}
  \lit $\pr{\sum(\gamma_i * a_i = O)} \implies (\Gamma = \{0\}^{|A|})$ \pipe $LinInd[A, V, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SuperDependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  \pr{(VectorSpace[V, +, *]) \land (A \subseteq B \subseteq V) \land (|B| \in \mathbb{N})} \hfill \implies \hfill \\
  \pr{(\lnot LinInd[A, V, +, *]) \implies (\lnot LinInd[B, V, +, *])}
\end{array}\right)$
\begin{enumerate}
  \lit $U \abr B \setminus A$
  \lit $\sum(0 * u_i) = O$
  \lit $\pr{\exists_{\Gamma \in \mathbb{R}^|A|}\pr{\pr{\sum(\gamma_i * a_i) = O} \land (\Gamma \neq \{0\}^{|A|})}}$
  \lit $\pr{\sum(\gamma_i * b_i) = \sum(\gamma_i * a_i) + O = O + O = O} \land (\Gamma' = \Gamma \times \{0\}^{|U|} \neq \{0\}^|B|)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$UniqueLinComb \abr \forall_{S, V}\pr{(Basis[S, V]) \implies \pr{\forall_{v, \Gamma, \Delta}\pr{\pr{\pr{v = \sum(\gamma_i * u_i)} \land \pr{v = \sum(\delta_i * u_i)}} \implies (\Gamma = \Delta)}}}$
\begin{enumerate}
  \lit $(v \in V) \implies \ldots$
  \begin{enumerate}
    \lit $Spans[V, S, +, *]$ \pipe $\pr{\exists_{\Gamma \in \mathbb{R}^n}\pr{v = \sum(\gamma_i * s_i)}} \land \pr{\exists_{\Gamma \in \mathbb{R}^n}\pr{v = \sum(\gamma_i * s_i)}}$
    \lit $O = v - v = \sum(\gamma_i * s_i) - \sum(\delta_i * s_i) = \sum\pr{(\gamma_i - \delta_i) * s_i}$ \pipe $\sum\pr{(\gamma_i - \delta_i) * s_i} = O$
    \lit $(LinInd[S, V, +, *]) \land \pr{\sum\pr{(\gamma_i - \delta_i) * s_i} = O}$ \pipe $\{\gamma_i - \delta_i\} = \{0\}^n$ \pipe $\{\gamma_i\} = \{\delta_i\}$ \pipe $\Gamma = \Delta$
  \end{enumerate}
  \lit $\Gamma = \Delta$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$BasisSubSpan \abr \forall_{S, V}\pr{(Spans[S, V, +, *]) \implies \pr{\exists_{B \subseteq S}(Basis[B, V, +, *])}}$
\begin{enumerate}
  \lit $A = B$
...
$BasisCardProp \abr \forall_{S, T, V}\pr{\pr{(Basis[S, V, +, *]) \land (Basis[T, V, +, *])} \implies (|T| = |S|)}$
\begin{enumerate}
