...
$SubspaceEquiv \abr \forall_{V : VectorSpace[V, +, *]} \forall_{S : \emptyset \neq S \subseteq V}\pr{(Subspace[S, V, +, *]) \iff \pr{\pr{\forall_{r, s \in S}(r + s \in S)} \land \pr{\forall_{\alpha \in \mathbb{R}} \forall_{s \in S}(\alpha * s \in S)}}}$
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \lit $u \in S$ \pipe $0 * u = O \in S$
      \lit $u \in V$ \pipe $u + O = u$
      \lit $u \in S$ \pipe $(-1) * u = -u \in S$
    ...
    \end{enumerate}
    \lit $\pr{(\alpha ,\beta \in \mathbb{R}) \land (u, v, w \in S)} \implies \ldots$
    \lit $(VectorSpace[S, +, *]) \land (\emptyset \neq S \subseteq V) \land (VectorSpace[V, S, +, *])$ \pipe $Subspace[S, V, +, *]$
  \end{enumerate}
...
$LinComb[c, U, V, +, *] \abr (VectorSpace[V, +, *]) \land (U \subseteq V) \land (n = |U| \in \mathbb{N}) \land \pr{\exists_{\Gamma \in \mathbb{R}^{n}}\pr{c = \sum_{i = 1}^{n}(\gamma_i * u_i)}}$ \\
$LinSpan[S, U, V, +, *] \abr \left(\begin{array}{ll}
  (VectorSpace[V, +, *]) \hfill \land \hfill (U \subseteq V) \hfill \land \hfill (|U| \in \mathbb{N}) \hfill \land \hfill \pr{(U = \emptyset) \implies (S = \{O\})} \hfill \land \hfill \\
  \pr{(U \neq \emptyset) \implies (S = \{c \in V | LinComb[c, U, V, +, *]\})}
\end{array}\right)$ \\
...
\begin{enumerate}
  \lit $A = B$
  \lit While $\lnot LinearlyIndependent(A, V, +, *)$, $\exists_{j \in A}(LinearCombination[j, A \setminus \{j\}, +, *])$, $A' = A \setminus \{j\}$
  \lit $Spans[A', S, +, *]$, until $(LinearlyIndependent[A', V, +, *]) \land (Spans[A', V, +, *])$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$BasisCard \abr \forall_{S, T, V}\pr{\pr{(Basis[S, V, +, *]) \land \pr{LinearlyIndependent(T, V, +, *)}} \implies (|T| \leq |S|)}$
\begin{enumerate}
  ...
  \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \lit \cont $t_j = \sum(\gamma'_i * t_i)$ \pipe $\lnot LinearlyIndependent(T, V, +, *)$
      \end{enumerate}
      \lit $(|T| > |S|) \implies \pr{\lnot LinearlyIndependent(T, V, +, *)}$ \pipe $\pr{LinearlyIndependent(T, V, +, *)} \implies (|T| \leq |S|)$
   \end{enumerate}
  \lit $\pr{(Basis[S, V, +, *]) \land \pr{LinearlyIndependent(T, V, +, *)}} \implies (|T| \leq |S|)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

TODO: CONTINUE COROLLARY PAGE 15
