...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $(LinComb[\alpha, A, K_\alpha, V, +_v, *_v]) \land (LinComb[\beta, A, K_\beta, V, +_v, *_v])$
  ...
  \begin{enumerate}
    \lit $LinComb[\alpha, A, K, V, +_v, *_v]$
    \lit $L(r *_v \alpha) = L\pr{r *_v \sum_{i = 1}^{n}(k_i *_v a_i)} = L\pr{\sum_{i = 1}^{n}\pr{(r k_i) *_v a_i}} = \sum_{i = 1}^{n}\pr{(r k_i) *_w b_i} = r *_w $
  \end{enumerate}
