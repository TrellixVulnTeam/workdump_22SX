...
\begin{shaded} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  $SetSum[A + B, A, B, V, +, *] \abr (VectorSpace[V, +, *]) \land (A, B \subseteq V) \land \pr{A + B = \{a + b \st (a \in A) \land (b \in B)\}}$ \\
...
$NullSpace[N, A, m, n] \abr (Matrix[A, m, n]) \land (N = \{x \in \mathbb{R}^n \st A * x = O\})$ \\  % space of homogenous solutions
$RowSpace[R, A, m, n] \abr (Matrix[A, m, n]) \land (R = \{x^T * A \in \mathbb{R}^n \st x \in \mathbb{R}^m\})$ \\  % space of linear combination of rows
$ColSpace[C, A, m, n] \abr (Matrix[A, m, n]) \land (C = \{A * x \in \mathbb{R}^m \st x \in \mathbb{R}^n\})$ \\  % space of linear combination or columns
...
$LinSpan[S', S, V, +, *] \abr \left(\begin{array}{ll}
  ...
  \pr{(S \neq \emptyset) \implies \pr{S' = \{c \in V \st (K \in \mathbb{R}^n) \land (LinComb[c, S, K, V, +, *])\}}}
\end{array}\right)$ \\
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \lit $\pr{a + b = \sum_{i = 1}^{n}\pr{({k_a}_i + {k_b}_i) * s_i}} \land (\langle {k_a}_i + {k_b}_i \st i \in \mathbb{N}_{1, n} \rangle \in \mathbb{R}^n)$ \cont
      ...
      \begin{enumerate}
        ...
        \lit $\pr{r * u = \sum_{i = 1}^{n}(r k_i) * s_i}) \land (\langle r k_i \in \mathbb{R} \st i \in \mathbb{N}_{1, n} \rangle \in \mathbb{R}^n)$ \pipe $\exists_{M \in \mathbb{R}^n}\pr{r * u = \sum_{i = 1}^{n}(m_i * s_i)}$ 
      ...
      \begin{enumerate}
        \lit $K_s \abr \langle \left(\begin{cases} 
          1 & j = i \\
          0 & j \neq i 
        \end{cases}\right) \st j \in \mathbb{N}_{1, n} \rangle$ \pipe $(K_s \in \mathbb{R}^n) \land \pr{\sum_{j = 1}^{n}({k_s}_j * s_j) = s_i}$
...
$Ker[ker_L, L, V, +_v, *_v, W, +_w, *_w] \abr (LinTrans[L, V, +_v, *_v, W, +_w, *_w]) \land \pr{ker_L = \{\alpha \in V \st L(\alpha) = O_w\}}$ \\
...
$Rng[rng_L, L, V, +_v, *_v, W, +_w, *_w] \abr (LinTrans[L, V, +_v, *_v, W, +_w, *_w]) \land \pr{rng_L = \{\beta \in W \st \exists_{\alpha \in V}\pr{\beta = L(\alpha)}\}}$ \\
...
$KerInj \abr \forall_{L, V, W}\pr{(Ker[ker_L, L, V, +_v, *_v, W, +_w, *_w]) \implies \pr{(Inj[L, V, W]) \iff (ker_L = \{O_v\})}}$
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      \lit $L(v) = O_w$
      \lit $(Inj[L, V, W]) \land \pr{L(O_v) = O_w}$ \pipe $O_v = v$
    \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \end{enumerate}
    \lit $\pr{(u, v \in V) \land \pr{L(u) = L(v)}} \implies (u = v)$ \pipe $\forall_{u, v \in V}\pr{\pr{L(u) = L(v)} \implies (u = v)}$ \pipe $Inj[L, V, W]$
  \end{enumerate}
  \lit $(ker_L = \{O_v\}) \implies (Inj[L, V, W])$
  \lit $(Inj[L, V, W]) \iff (ker_L = \{O_v\})$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$RngSurj \abr \forall_{L, V, W}\pr{(Ran[rng_L, L, V, +_v, *_v, W, +_w, *_w]) \implies \pr{(Surj[L, V, W]) \iff (rng_L = W)}}$
\begin{enumerate}
  \lit $(SurjEquiv) \land \pr{rng(L) = rng_L}$ \pipe $(Surj[L, V, W]) \iff (rng_L = W)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$InjSurjEqualDim \abr \forall_{T, V, W}\left(\begin{array}{ll}
  \pr{(LinTrans[T, V, +_v, *_v, W, +_w, *_w]) \land (Dim[V] = Dim[W]) \land (Inj[T, V, W])} \hfill \implies \hfill \\
  (Surj[T, V, W])
\end{array}\right)$
\begin{enumerate}
...
$SurjInjEqualDim \abr \forall_{T, V, W}\left(\begin{array}{ll}
  \pr{(LinTrans[T, V, +_v, *_v, W, +_w, *_w]) \land (Dim[V] = Dim[W]) \land (Surj[T, V, W])} \hfill \implies \hfill \\
  (Inj[T, V, W])
\end{array}\right)$
\begin{enumerate}
...
$SmallerMapNotInj \abr \forall_{T, V, W}\pr{\pr{(LinTrans[T, V, +_v, *_v, W, +_w, *_w]) \land (Dim[V] > Dim[W])} \implies (\lnot Inj[T, V, W])}$
\begin{enumerate}
...
$LargerMapNotSurj \abr \forall_{T, V, W}\pr{\pr{(LinTrans[T, V, +_v, *_v, W, +_w, *_w]) \land (Dim[V] < Dim[W])} \implies (\lnot Surj[T, V, W])}$
\begin{enumerate}
...
$InvertibleBijEquiv \abr \forall_{L}\pr{(LTInvertible[L, V, +_v, *_v, W, +_w, *_w]) \iff \pr{(Inj[L, V, W]) \land (Surj[L, V, W])}}$
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \end{enumerate}
    \lit $\pr{L(u) = L(w)} \implies (u = w)$ \pipe $\forall_{u, w}\pr{\pr{L(u) = L(w)} \implies (u = w)}$ \pipe $Inj[L, V, W]$
    ...
    \end{enumerate}
    \lit $(w \in W) \implies \pr{\exists_{v \in V}\pr{w = \pr{L(v)}}}$ \pipe $\forall_{w \in W} \exists_{v \in V}\pr{w = L(v)}$ \pipe $Surj[L, V, W]$
    \lit $(Inj[L, V, W]) \land (Surj[L, V, W])$
  \end{enumerate}
  \lit $(LTInvertible[L, V, +_v, *_v, W, +_w, *_w]) \implies \pr{(Inj[L, V, W]) \land (Surj[L, V, W])}$
  \lit $\pr{(Inj[L, V, W]) \land (Surj[L, V, W])} \implies \ldots$
  \begin{enumerate}
    \lit $(Inj[L, V, W]) \land (Surj[L, V, W])$ \pipe $\forall_{w \in W} \exists!_{v \in V}\pr{w = L(v)}$
    \lit $S \abr \{(w, v) \in W \times V \st w = L(v)\}$
    \lit $\pr{\forall_{w \in W} \exists!_{v \in V}\pr{w = L(v)}} \land \pr{S = \{(w, v) \in W \times V \st w = L(v)\}}$ \pipe $Function[S, W, V]$
    ...
    \begin{enumerate}
      ...
      \lit $(Inj[L, V, W]) \land \pr{L\pr{S(w_1) + S(w_2)} = L\pr{S(w_1 + w_2)}}$ \pipe $S(w_1) + S(w_2) = S(w_1 + w_2)$
    \end{enumerate}
    ...
    \begin{enumerate}
      ...
      \lit $(Inj[L, V, W]) \land \pr{L\pr{r * S(w)} = L\pr{S(r * w)}}$ \pipe $r * S(w) = S(r * w)$
    \end{enumerate}
  ...
  \end{enumerate}
  \lit $\pr{(Inj[L, V, W]) \land (Surj[L, V, W])} \implies (LTInvertible[L, V, +_v, *_v, W, +_w, *_w])$
  \lit $(LTInvertible[L, V, +_v, *_v, W, +_w, *_w]) \iff \pr{(Inj[L, V, W]) \land (Surj[L, V, W])}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
...
$LTMatrix \abr \forall_{L, V, W}\left(\begin{array}{ll}
  ...
  \pr{\forall_{v \in V}\pr{CoordVec[[L(v)]_B, L(v), B, W, +_w, *_w] = \langle [L(a_i)]_B \st a_i \in A \rangle * CoordVec[[v]_A, v, A, V, +_v, *_v]}}
\end{array}\right)$
\begin{enumerate}
...
$TransitionMatrix \abr \forall_{L, V}\left(\begin{array}{ll}
  ...
  \pr{\forall_{v \in V}(CoordVec[[v]_B, v, B, W, +_w, *_w] = \langle [a]_B \st a \in A \rangle * CoordVec[[v]_A, v, A, V, +_v, *_v])}
\end{array}\right)$
\begin{enumerate}
...
% Permutations: Humphreys; 90-100
% Determinants: Lal; 215-223, 49-57
\begin{comment}
  ...
  $IntPermSetEquiv \abr (IntPermSet[\mathcal{S}_n, n]) \land (\tau \in \mathcal{S}_n) \implies (\mathcal{S}_n = \{\tau \circ \sigma \st \sigma \in \mathcal{S}_n\} = \{\sigma \circ \tau \st \sigma \in \mathcal{S}_n\} = \{\sigma^{-1} \st \sigma \in \mathcal{S}_n\})$
  \begin{enumerate}
    \lit $(\alpha \in \mathcal{S}_n) \iff \pr{\alpha = \tau \circ (\tau^{-1} \circ \alpha) \in \mathcal{S}_n} \iff (\alpha \in \{\tau \circ \sigma \st \sigma \in \mathcal{S}_n\})$
    \lit $(\alpha \in \mathcal{S}_n) \iff \pr{\alpha = (\alpha \circ \tau^{-1}) \circ \tau} \in \mathcal{S}_n) \iff (\alpha \in \{\sigma \circ \tau \st \sigma \in \mathcal{S}_n\})$
    \lit $(\alpha \in \mathcal{S}_n) \iff \pr{\alpha = (\alpha^{-1})^{-1} \in \mathcal{S}_n} \iff (\alpha \in \{\sigma^{-1} \st \sigma \in \mathcal{S}_n\})$
    \lit $\mathcal{S}_n = \{\tau \circ \sigma \st \sigma \in \mathcal{S}_n\} = \{\sigma \circ \tau \st \sigma \in \mathcal{S}_n\} = \{\sigma^{-1} \st \sigma \in \mathcal{S}_n\}$
  \end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

  $Inversions[n(\sigma), \sigma] \abr (\sigma \in \mathcal{S}_n) \land \pr{n(\sigma) = |\{(i, j) \st (i < j) \land \pr{\sigma(i) > \sigma(j)}\}| = \sum_{i = 1}^{n}\pr{|\{(i, j) \st \pr{\sigma(i) > \sigma(j)} \land (i < j \leq n)\}|}}$ \\
  $Transposition[\tau, m, r] \abr (\tau \in \mathcal{S}_n) \land \pr{\forall_{i \in \mathbb{N}_{1, n}}\pr{(i \notin \{m, r\}) \implies \pr{\tau(i) = i}}} \land \pr{\tau(m) = r} \land \pr{\tau(r) = m}$ \\

  $TranspositionInv \abr (Transposition[\tau]) \implies (\tau = \tau^{-1})$
  \begin{enumerate}
    \lit $\tau \circ \tau^{-1} = I_n$ \pipe $\tau = \tau^{-1}$
  \end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

  $TranspositionDecomp \abr (\sigma \in \mathcal{S}_n) \implies \pr{\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma = \circ_{\tau_i \in T}(\tau_i)}}}$
  \begin{enumerate}
    \lit $\pr{n(\sigma) = 0} \implies \ldots$
    \begin{enumerate}
      \lit $\sigma = I_n$
      \lit $\pr{Transpositions[\langle (1 2), (1 2) \rangle]} \land \pr{\sigma = I_n = (1 2) \circ (1 2)}$
      \lit $\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma = \circ_{\tau_i \in T}(\tau_i)}}$
    \end{enumerate}
    \lit $\pr{n(\sigma) = 0} \implies \pr{\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma = \circ_{\tau_i \in T}(\tau_i)}}}$
    \lit $\pr{\pr{n(\sigma) = k + 1} \land \pr{\forall_{\sigma' \in \mathcal{S}_n}\pr{\pr{n(\sigma') \leq k} \implies \pr{\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma' = \circ_{\tau_i \in T}(\tau_i)}}}}}} \implies \ldots$
    \begin{enumerate}
      \lit $n(\sigma) = k + 1$ \pipe $n(\sigma) > 1$ \pipe $\exists_{l \in \mathbb{N}}\pr{\pr{\sigma(l) \neq l} \land \pr{\forall_{i \in \mathbb{N}_{1, l}}\pr{\sigma(i) = i}}}$
      \lit $Transposition[\delta, l, \tau(l)]$ \pipe $n(\delta \circ \sigma) < n(\sigma)$
      \lit $\forall_{\sigma' \in \mathcal{S}_n}\pr{\pr{n(\sigma') \leq k} \implies \pr{\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma' = \circ_{\tau_i \in T}(\tau_i)}}}}$ \pipe $\exists_{T'}\pr{(Transpositions[T']) \land \pr{\delta \circ \sigma = \circ_{\tau_i \in T'}(\tau_i)}}$
      \lit $\sigma = \delta^{-1} \circ \circ_{\tau_i \in T'}(\tau_i) = \delta \circ \circ_{\tau_i \in T'}(\tau_i) = \circ_{\tau_i \in T''}(\tau_i)$
      \lit $\exists_{T''}\pr{(Transpositions[T'']) \land \pr{\sigma = \circ_{\tau_i \in T''}(\tau_i)}}$
    \end{enumerate}
    \lit $\pr{\pr{n(\sigma) = k + 1} \land \pr{\forall_{\sigma' \in \mathcal{S}_n}\pr{\pr{n(\sigma') \leq k} \implies \pr{\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma' = \circ_{\tau_i \in T}(\tau_i)}}}}}} \implies$ \cont
    \lit \cont $\pr{\exists_{T''}\pr{(Transpositions[T'']) \land \pr{\sigma = \circ_{\tau_i \in T''}(\tau_i)}}}$
    \lit $Induction$ \pipe $\exists_{T}\pr{(Transpositions[T]) \land \pr{\sigma = \circ_{\tau_i \in T}(\tau_i)}}$
  \end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
\end{comment}
...
\begin{enumerate}
  ...
  \end{enumerate}
  \lit $(|\mathcal{S}_m| = m!) \implies \pr{|\mathcal{S}_{m + 1}| = (m + 1)!}$
  \lit $\mathcal{S}_n = \{\sigma \st Perm[\sigma, \mathbb{N}_{1, n}]\}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$Inversion[\langle \sigma_i, \sigma_j \rangle, \sigma] \abr (\sigma \in \mathcal{S}_n) \land (i < j \leq n) \land \pr{\sigma(i) > \sigma(j)}$ \\
$Signature[sgn(\sigma), \sigma] \abr (\sigma \in \mathcal{S}_n) \land (v = |\{\langle \sigma_i, \sigma_j \rangle \st Inversion[\langle \sigma_i, \sigma_j \rangle, \sigma]\}|) \land \pr{sgn(\sigma) = (-1)^v}$ \\
$Determinant[det(A), A, n] \abr det(A) = \sum_{\sigma \in \mathcal{S}_n}^{}\pr{sgn(\sigma) \prod_{i = 1}^{n}\pr{a_{i, \sigma(i)}}}$ \\
...
$DetEquiv \abr \pr{\forall_{i \in \mathbb{N}_{1, n}}\pr{det(A) = \sum_{j = 1}^{n}(a_{i j} A_{i, j})}} \land \pr{\forall_{j \in \mathbb{N}_{1, n}}\pr{det(A) = \sum_{i = 1}^{n}(a_{i j} A_{i, j})}}$
\begin{enumerate}
  \lit TODO
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
