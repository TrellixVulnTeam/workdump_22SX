...
$LinearCombination[c, U, V, +, *] \abr (VectorSpace[V, +, *]) \land (\emptyset \neq U \subseteq V) \land (|U| \in \mathbb{N}) \land \pr{\exists_{\Gamma \in \mathbb{R}^{|U|}}\pr{c = \sum_{i = 1}^{|U|}(\gamma_i * u_i)}}$ \\
$Linear????$
...
$LinearlyIndependent[S, V, +, *] \abr LinearCombination[O, S, V, +, *]$ \\
??????????????? how to say
