...
$+(A, B) \abr \pr{\pr{Matrix(A, m, n)} \land \pr{Matrix(B, m, n)}} \implies (A + B = [a_{i, j} + b_{i, j}]_{m \times n}) $ \\
$*(r, A) \abr \pr{(r \in \mathbb{R}) \land \pr{Matrix(A, m, n)}} \implies (r * A = [r a_{i, j}]_{m \times n})$ \\
$*(A, B) \abr \pr{\pr{Matrix(A, m, p)} \land \pr{Matrix(B, p, n)}} \implies \pr{A * B = \left[\sum_{k = 1}^{p}(a_{i, k} b_{k, j})\right]_{m \times n}}$ \\
$\phantom{ }^T(A) \abr \pr{Matrix(A, m, n)} \implies (A^T = [a_{j, i}]_{n \times m})$ \\

$AddCom \abr \forall_{A, B \in \mathcal{M}}(A + B = B + A)$ \pipe $\Leftarrow$ $A + B = [a_{i, j} + b_{i, j}] = [b_{i, j} + a_{i, j}] = B + A$ \\
$AddAssoc \abr \forall_{A, B, C \in \mathcal{M}}\pr{(A + B) + C = A + (B + C)}$ \pipe $\Leftarrow$ $(A + B) + C = [(a_{i, j} + b_{i, j}) + c_{i, j}] = [a_{i, j} + (b_{i, j} + c_{i, j})] = A + (B + C)$ \\
...
$AddInv \abr \forall_{A \in \mathcal{M}} \exists!_{(-A) \in \mathcal{M}}\pr{A + (-A) = O = (-A) + A}$    \phantom{TODO}--- \\ % \pipe $\Leftarrow$ $A + (-A) = [a_{i, j} - a_{i, j}] = O = [-a_{i, j} + a_{i, j}] = (-A) + A$ \\

$MulAssoc \abr \forall_{A, B, C \in \mathcal{M}}\pr{(A * B) * C = A * (B * C)}$ \pipe $\Leftarrow$
\begin{enumerate}
...
$LeftDist \abr \forall_{A, B, C \in \mathcal{M}}\pr{(A + B) * C = A * C + B * C}$ \pipe $\Leftarrow$
\begin{enumerate}
...
$RightDist \abr \forall_{A, B, C \in \mathcal{M}}\pr{C * (A + B) * C = C * A + C * B}$    \phantom{TODO}--- \\
$Scalar1 \abr \forall_{r, s \in \mathbb{R}} \forall_{A \in \mathcal{M}}\pr{r (s A) = (r s) A = s (r A)}$    \phantom{TODO}--- \\
$Scalar2 \abr \forall_{r \in \mathbb{R}} \forall_{A, B \in \mathcal{M}}\pr{A (r B) = r (A * B)}$    \phantom{TODO}--- \\
$Scalar3 \abr \forall_{r, s \in \mathbb{R}} \forall_{A \in \mathcal{M}}\pr{(r + s) A = r A + s A}$    \phantom{TODO}--- \\
$Scalar4 \abr \forall_{r \in \mathbb{R}} \forall_{A, B \in \mathcal{M}}\pr{r (A + B) = r A + r B}$    \phantom{TODO}--- \\

$Trans1 \abr \forall_{A \in \mathcal{M}}\pr{A = (A^T)^T}$ \pipe $\Leftarrow$ $A = [a_{i, j}] = [a_{j, i}]^T = ([a_{i, j}]^T)^T = (A^T)^T$ \\
$Trans2 \abr \forall_{A, B \in \mathcal{M}}\pr{(A + B)^T = A^T + B ^T}$    \phantom{TODO}--- \\
$Trans3 \abr \forall_{A, B \in \mathcal{M}}\pr{(A * B)^T = B^T * A^T}$ \pipe $\Leftarrow$ 
\begin{enumerate}
...
$Sym(A) \abr A = A^T$ \\
$SkewSym(A) \abr A = -A^T$ \\
$SymGen \abr \forall_{A \in \mathcal{M}}\pr{Sym(A + A^T)}$ \pipe $\Leftarrow$ $(A + A^T)^T = A^T + (A^T)^T = A^T + A = A + A^T$ \\
$SkewSymGem \abr \forall_{A \in \mathcal{M}}\pr{SkewSym(A - A^T)}$ \pipe $\Leftarrow$ $-(A - A^T)^T = -\pr{A^T - (A^T)^T} = -(A^T - A) = (A - A^T)$ \\
$SymDec \abr \forall_{A \in \mathcal{M}} \exists!_{B : Sym(B)} \exists!_{C : SkewSym(C)}\pr{A = B + C = (1 / 2) * (A + A^T) + (1 / 2) * (A - A^T)}$    \phantom{TODO}--- \\

$Square(A, n) \abr Matrix(A, n, n)$ \\
$UpperTriangular(A) \abr \pr{Square(A)} \land (i > \implies a_{i, j} = 0)$ \\
$LowerTriangular(A) \abr \pr{Square(A)} \land (i < j \implies a_{i, j} = 0)$ \\
$Diagonal(A, n) \abr \pr{Square(A, n)} \land (i \neq j \implies a_{i, j} = 0)$ \\
$Scalar(A, n, k) \abr \pr{Diagonal(A, n)} \land (a_{i, i} = k)$ \\
$I_n \abr Scalar(I, n, 1)$ \\
$MulId \abr \forall_{A : Square(A, n)}(A * I_n = A = I_n * A)$    \phantom{TODO}--- \\

$Invertible(A) \abr \pr{Square(A, n)} \land \pr{\exists_{A^{-1} \in \mathcal{M}}(A * A^{-1} = I_n = A^{-1} * A)}$ \\
$Inv1 \abr \forall_{A : Invertible(A)}\pr{\exists!_{A^{-1} \in \mathcal{M}}(A * A^{-1} = I_n = A^{-1} * A)}$    \phantom{TODO}--- \\
$Inv2 \abr \forall_{A : Invertible(A)} \forall_{B : Invertible(B)}\pr{(A * B)^{-1} = B^{-1} * A^{-1}}$ \pipe $\Leftarrow$
\begin{enumerate}
  \lit $(A * B) * (A * B)^{-1} = I$ \pipe $B * (A * B)^{-1} = A^{-1}$ \pipe $(A * B)^{-1} = B^{-1} * A^{-1}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$Inv3 \abr \forall_{A : Invertible(A)}\pr{(A^{-1})^{-1} = A}$    \phantom{TODO}--- \\
$InvTrans \abr \forall_{A : Invertible(A)}\pr{(A^T)^{-1} = (A^{-1})^T}$ \pipe $\Leftarrow$
\begin{enumerate}
  \lit $A^T * (A^{-1})^T = (A^{-1} * A)^T = I^T = I$ \pipe $(A^{-1})^T = (A^T)^{-1}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$Sys(A, X, B) \abr (A * X = B) \land \pr{Matrix(A, m, n)} \land \pr{Matrix(X, n, 1)} \land \pr{Matrix(B, m, 1)}$ \\
$Sol(X, A, B) \abr Sys(A, X, B)$ \\
$TrivSol(X, A, B) \abr \pr{Sol(X, A, B)} \land (x_{i, 1} = 0)$ \\

$RREF(A) \abr (Definition 1.18)$ \\
$RowOp(\phi) \abr (Definition 1.19)$ \\
$RowEquiv(A, B) \abr \exists_{\Phi : |\Phi| \in \mathbb{N}}\pr{\pr{\forall_{\phi \in \Phi}\pr{RowOp(\phi)}} \land \pr{\Phi(A) = B}}$ \\

$Elem(A, n) \abr RowEquiv(A, I_n)$ \\
$RowOpIsElem \abr \forall_{\phi : RowOp(\phi)}\pr{\pr{E = \phi(I)} \implies \pr{\phi(A) = E * A}}$ \\
...
$GaussJordan \abr \forall_{A} \exists!_{B}\pr{RREF(B) \land RowEquiv(A, B)}$    \phantom{TODO}--- \\
