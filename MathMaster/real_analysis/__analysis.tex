...
$LinearCombination[c, U, V, +, *] \abr (VectorSpace[V, +, *]) \land (U \subseteq V) \land (n = |U| \in \mathbb{N}) \land \pr{\exists_{\Gamma \in \mathbb{R}^{n}}\pr{c = \sum_{i = 1}^{n}(\gamma_i * u_i)}}$ \\
$LinearSpan[S, U, V, +, *] \abr \left(\begin{array}{ll}
  ...
  \pr{(U \neq \emptyset) \implies (S = \{c \in V | LinearCombination[c, U, V, +, *]\})}
\end{array}\right)$ \\
$Spans[S, V, +, *] \abr LinearSpan[V, S, V, +, *]$ \\
$LinearlyIndependent[U, V, +, *] \abr \left(\begin{array}{ll}
...
$SingletonZeroDependent \abr \lnot LinearlyIndependent[\{O\}, V, +, *]$
\begin{enumerate}
  \lit $(1 * O = O) \land (1 \neq 0)$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SingletonNonZeroIndependent \abr (\alpha \neq O) \implies (LinearlyIndependent[\{\alpha\}, V, +, *])$
\begin{enumerate}
...
$SubIndependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  ...
  \pr{(LinearlyIndependent[B, V, +, *]) \implies (LinearlyIndependent[A, V, +, *])}
\end{array}\right)$
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \lit $LinearlyIndependent[B, V, +, *]$ \pipe $\Gamma' = \{0\}^{|B|}$
    \lit $\Gamma' = \{0\}^{|B|} = \Gamma \times \{0\}^{|U|}$ \pipe $\Gamma = \{0\}^{|A|}$
  \end{enumerate}
  \lit $\pr{\sum(\gamma_i * a_i = O)} \implies (\Gamma = \{0\}^{|A|})$ \pipe $LinearlyIndependent[A, V, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 

$SuperDependent \abr \forall_{V, A, B}\left(\begin{array}{ll}
  \pr{(VectorSpace[V, +, *]) \land (A \subseteq B \subseteq V) \land (|B| \in \mathbb{N})} \hfill \implies \hfill \\
  \pr{(\lnot LinearlyIndependent[A, V, +, *]) \implies (\lnot LinearlyIndependent[B, V, +, *])}
\end{array}\right)$
\begin{enumerate}
  \lit $U \abr B \setminus A$
  \lit $\sum(0 * u_i) = O$
...
$LinearlyIndependentEquiv \abr \forall_{U, V}\pr{(LinearlyIndependent[U, V, +, *]) \iff \pr{\forall_{j \in U}(\lnot LinearCombination[j, U \setminus \{j\}, +, *])}}$
\begin{enumerate}
  \lit $(\lnot LinearlyIndependent[U, V, +, *]) \implies \ldots$
  \begin{enumerate}
    \lit 123123
   \end{enumerate}
  \lit $(\lnot LinearlyIndependent[U, V, +, *]) \implies \pr{\exists_{j \in U}(LinearCombination[j, U \setminus \{j\}, +, *])}$ \pipe $\pr{\forall_{j \in U}(\lnot LinearCombination[j, U \setminus \{j\}, +, *])} \implies (LinearlyIndependent[U, V, +, *])$
  \lit $\pr{\exists_{j \in U}(LinearCombination[j, U \setminus \{j\}, +, *])} \implies \ldots$
  \begin{enumerate}
    \lit 123123
  \end{enumerate}
  \lit $\pr{\exists_{j \in U}(LinearCombination[j, U \setminus \{j\}, +, *])} \implies (\lnot LinearlyIndependent[U, V, +, *])$ \pipe $(LinearlyIndependent[U, V, +, *]) \implies \pr{\forall_{j \in U}(\lnot LinearCombination[j, U \setminus \{j\}, +, *])}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 


$Basis[S, V, +, *] \abr (Spans[S, V, +, *]) \land (LinearlyIndependent[S, V, +, *])$ \\



$Dimension[d, V, +, *] \abr DO REDUCTION ALGORITHM! $
