...
$SmallestSubspaceContaining \abr \forall_{T, S', S, V}\pr{\pr{(LinSpan[S', S, V, +, *]) \land (Subspace[T, V, +, *]) \land (S \subseteq T)} \implies (S' \subseteq T)}$
\begin{enumerate}
  \lit $LSSubspaceContaining$ \pipe $(Subspace[S', V, +, *]) \land (S \subseteq S')$
  \lit $VectorSpace[S', +, *]$

  \lit $(W \subseteq V) \implies (LinSpan[W, W, V, +, *] \iff Subspace[W, V, +, *])$
  \lit $\pr{(W \subseteq V) \land (Subspace[W, V, +, *]) \land (S \subseteq W) \land (LinSpan[S', S, W, +, *])} \implies (Subspace[S', W, +, *])$

  \lit $\forall_{T \subset S'}\pr{(\lnot Subspace[T, V, +, *]) \lor (S \not \subseteq T)}$
  \lit $\lnot \exists_{T \subset S'}\pr{(Subspace[T, V, +, *]) \land (S \subseteq T)}$
...
$NullSpace[N, A] \abr (Matrix[A, m, n]) \land (N = \{x \in \mathbb{R}^n | A * x = O\})$ \\  % space of homogenous solutions
$RowSpace[R, A] \abr (Matrix[A, m, n]) \land (R = \{x^T * A | x \in \mathbb{R}^m\})$ \\  % space of linear combination of rows
$ColSpace[C, A] \abr (Matrix[A, m, n]) \land (C = \{A * x | x \in \mathbb{R}^n\})$ \\  % space of linear combination or columns
