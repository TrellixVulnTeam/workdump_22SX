...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $(Subspace[A, V, +, *]) \land (Subspace[B, V, +, *])$ \pipe $(O \in A) \land (O \in B)$ \pipe $O \in A \oplus B$
  ...
  \begin{enumerate}
    \lit $(s \in A + B) \implies \ldots$
    \begin{enumerate}
      \lit $s \in A + B$ \pipe $\pr{\exists_{\langle a, b \rangle \in A \times B}(s = a + b)}$
      \lit $\pr{(s = a_1 + b_1) \land (s = a_2 + b_2)} \implies \ldots$
      \begin{enumerate}
        \lit $O = s - s = (a_1 + b_1) - (a_2 + b_2) = (a_1 - a_2) + (b_1 - b_2)$
        \lit $(a_1 - a_2 \in A) \land (b_1 - b_2 \in B)$
...
$DirSumProp \abr \forall_{A, B, V}\left(\begin{array}{ll}
  \pr{(Subspace[A, V, +, *]) \land (Subspace[B, V, +, *]) \land (SetSum[A + B, A, B, V, +, *])} \hfill \implies \hfill \\
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      \lit $v \in A \cap B$ \pipe $(v \in A) \land (v \in B)$ \pipe $(v \in A) \land (-v \in B)$
      \lit $(v \in A) \land (-v \in B)$ \pipe $O = v + (-v) \in A + B$
      \lit $(DirSum[A \oplus B, A, B, V, +, *]) \land (O \in A + B)$ \pipe $\exists!_{\langle a, b \rangle \in A \times B}(O = a + b)$ \pipe $v = O$
    \end{enumerate}
...
$LinSpan(LinSpan[S', S, V, +, *]) \implies \pr{(Subspace[S', V, +, *]) \land (S \subseteq S')}$
\begin{enumerate}
...
$FiniteDim[V, +, *] \abr \exists_{S \in V^n}(Spans[S, V, +, *])$ \\
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $LSSubspaceContains$ \pipe $Subspace[W', V, +, *]$ \pipe $Subspace[W, V, +, *]$
  \end{enumerate}
...
\begin{enumerate}
  ...
  \lit $(LSSubspaceContains) \land (LinSpan[S', S, W, +, *])$ \pipe $Subspace[S', W, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
