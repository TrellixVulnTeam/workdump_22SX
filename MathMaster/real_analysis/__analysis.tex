...
$+(A, B) \abr (A + B = [a_{i, j} + b_{i, j}]_{m \times n}) \land \pr{Matrix(A, m, n)} \land \pr{Matrix(B, m, n)}$ \\
$*(r, A) \abr (r * A = [r a_{i, j}]_{m \times n}) \land \pr{Matrix(A, m, n)}$ \\
$*(A, B) \abr \pr{A * B = \left[\sum_{k = 1}^{p}(a_{i, k} b_{k, j})\right]_{m \times n}} \land \pr{Matrix(A, m, p)} \land \pr{Matrix(B, p, n)}$ \\
$\phantom{ }^T(A) \abr (A^T = [a_{j, i}]_{n \times m}) \land \pr{Matrix(A, m, n)}$ \\
...
$SkewSymGem \abr SkewSym(A - A^T)$ \pipe $\leftarrow$ $-(A - A^T)^T = -\pr{A^T - (A^T)^T} = -(A^T - A) = (A - A^T)$ \\
$SymDec \abr A = (1 / 2) * (A + A^T) + (1 / 2) * (A - A^T)$    \phantom{TODO}--- \\

$Square(A) \abr Matrix(A, n, n)$ \\
$Diagonal(A) \abr \pr{Square(A)} \land (i \neq j \implies a_{i, j} = 0)$ \\
$Scalar(A) \abr \pr{Diagonal(A)} \land (a_{i, i} = k)$ \\
$I \abr \pr{Scalar(I)} \land (i_{d, d} = 1)$ \\
...
$UniqueInverse \abr \pr{(A * B_1 = I) \land (A * B_2 = I)} \implies B_1 = B_2$    \phantom{TODO}--- \\
$Inv1 \abr \pr{Invertible(A) \land Invertible(B)} \implies (A * B)^{-1} = B^{-1} * A^{-1}$ \pipe $\leftarrow$
\begin{enumerate}
  \lit $(A * B) * (A * B)^{-1} = I$ \pipe $B * (A * B)^{-1} = A^{-1}$ \pipe $(A * B)^{-1} = B^{-1} * A^{-1}$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
$Inv2 \abr \pr{Invertible(A)} \implies \pr{\pr{Invertible(A^{-1}) \land (A = (A^{-1})^{-1}}}$    \phantom{TODO}--- \\
$InvTrans \abr \pr{Invertible(A)} \implies \pr{\pr{Invertible(A^T) \land ((A^T)^{-1} = (A^{-1})^T}}$ \pipe $\leftarrow$
\begin{enumerate}
...
$TrivSol(X, A, B) \abr \pr{Sol(X, A, B)} \land (a_{i. j} = 0)$ \\

$RREF(A) \abr (Definition 1.18)$ \\
$ElementaryRowOperation(\phi) \abr (Definition 1.19)$ \\
$RowEquiv(A, B) \abr \exists_{\Phi}\pr{\forall_{\phi \in \Phi}\pr{ElementaryRowOperation(\phi)} \land |\Phi| \in \mathbb{N} \land \Phi(A) = B}$ \\

$(A \neq O) \implies \pr{\exists_{B}\pr{RREF(B) \land RowEquiv(A, B)}}$ (By Gauss-Jordan Elimination)    \phantom{TODO}--- \\
$\pr{Sys(A, X_1, B) \land Sys(C, X_2, D) \land RowEquiv([A|B], [C|D])} \implies (X_1 = X_2)$ (By algebra on systems of equations)    \phantom{TODO}--- \\
$\pr{RowEquiv(A, B)} \implies \pr{Sys(A, X_1, O) \land Sys(B, X_1, O)}$ (Corollary)    \phantom{TODO}--- \\
$(m < n) \implies \pr{\exists_{X}\pr{\lnot TrivSol(X, A, O)}}$ \pipe $\pr{\forall_{X}(TrivSol(X, A, O)} \implies (m \geq n)$ \pipe $\leftarrow$
\begin{enumerate}
...
$\pr{ElementaryRowOperation(\phi) \land \pr{B = \phi(A)}} \implies \pr{Elem(E) \land B = E * A} $ \\
