...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \lit $A \subseteq B$ \pipe $\forall_{j \in \mathbb{N}_{1, n}}(a_j = b_j)$
    \lit CONTHERE
    \lit $\sum_{i = 1}^{n}(k_i * a_i)) = \sum_{i = 1}^{n}(k_i * a_i)) + O = \sum_{j = 1}^{m}(l_j * b_j))$
  ...
  \end{enumerate}
  \lit $\pr{(K \in \mathbb{R}^n) \land (LinComb[O, A, K, V, +, *])} \implies (K = \{0\}^n)$ \pipe $\forall_{K \in \subseteq \mathbb{R}^n}\pr{(LinComb[O, A, K, V, +, *]) \implies (K = \{0\}^n)}$ \pipe $LinInd[A, V, +, *]$
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
