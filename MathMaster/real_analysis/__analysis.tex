...
\end{enumerate} \vspace{.75mm} \hrule \vspace{.75mm} \ \\ 
CONTHERE
$HomoCompInduceHomo \abr \pr{(Homomorphism[\phi, G, *, H, \diamond]) \land (Homomorphism[\theta, H, \diamond, K, \mathbin{\square}])} \implies (Homomorphism[\theta \circ \phi, G, *, K, \mathbin{\square}])$
\begin{enumerate}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \lit $(ExpModOrder) \land (Order[n, G, *]) \land (Order[n, H, \diamond])$ \pipe $(g^{n_1} = g^{n_2}) \iff \pr{n_1 \equiv n_2 (mod \phantom{.} n)} \iff (h^{n_1} = h^{n_2}) \iff \pr{\phi(g^{n_1}) = \phi(g^{n_2})}$ \pipe $(g^{n_1} = g^{n_2}) \iff \pr{\phi(g^{n_1}) = \phi(g^{n_2})}$
  \end{enumerate}
